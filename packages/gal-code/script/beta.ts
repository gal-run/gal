...
const model = "gal-code/gpt-5.3-codex"
...
async function typecheck() {
  ...
  try {
    await $`bun typecheck`.cwd("packages/gal-code")
    return true
  } catch (err) {
...
async function build() {
  ...
  try {
    await $`./script/build.ts --single`.cwd("packages/gal-code")
    return true
  } catch (err) {
...
async function fix(pr: PR, files: string[], prs: PR[], applied: number[], idx: number) {
  console.log(`  Trying to auto-resolve ${files.length} conflict(s) with gal-code...`)
  ...
  const prompt = [
    ...
    "After resolving the conflicts, run `bun typecheck` in `packages/gal-code`.",
  ...
  try {
    await $`gal-code run -m ${model} ${prompt}`
  } catch (err) {
    console.log(`  gal-code failed: ${err}`)
    return false
  }
  ...
  console.log("  Conflicts resolved with gal-code")
  return true
}

async function smoke(prs: PR[], applied: number[]) {
  console.log("\nRunning final smoke check with gal-code...")

  const done = lines(prs.filter((x) => applied.includes(x.number)))
  const prompt = [
    "The beta merge batch is complete.",
    `Merged PRs on HEAD:\n${done}`,
    "Run `bun typecheck` in `packages/gal-code`.",
    "Run `./script/build.ts --single` in `packages/gal-code`.",
