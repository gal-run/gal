...
const GITHUB_REPO = "gal-run/gal-code"
const NPM_PACKAGE = "@gal-run/code"
...
async function fetchNpmDownloads(packageName: string): Promise<number> {
  try {
    ...
    const response = await fetch(
      `https://api.npmjs.org/downloads/range/2020-01-01:${endYear}-12-31/${encodedPackageName}`,
    )
    if (!response.ok) {
...
async function fetchReleases(): Promise<Release[]> {
  ...
  while (true) {
    const url = `https://api.github.com/repos/${GITHUB_REPO}/releases?page=${page}&per_page=${per}`
...
console.log("Fetching GitHub releases for gal-run/gal-code...\n")
