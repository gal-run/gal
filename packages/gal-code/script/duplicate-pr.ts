...
import { createGalCode } from "@scheduler-systems/gal-code-sdk"
...
`)
  ...
  const gal-code = await createGalCode({ port: 0 })
  ...
  try {
    ...
    const session = await gal-code.client.session.create()
    const result = await gal-code.client.session
      .prompt({
