...
import { and, Database, eq, isNull, lt, or, sql } from "@scheduler-systems/gal-code-console-core/drizzle/index.js"
import { KeyTable } from "@scheduler-systems/gal-code-console-core/schema/key.sql.js"
import { BillingTable, LiteTable, SubscriptionTable, UsageTable } from "@scheduler-systems/gal-code-console-core/schema/billing.sql.js"
import { centsToMicroCents } from "@scheduler-systems/gal-code-console-core/util/price.js"
import { getMonthlyBounds, getWeekBounds } from "@scheduler-systems/gal-code-console-core/util/date.js"
import { Identifier } from "@scheduler-systems/gal-code-console-core/identifier.js"
import { Billing } from "@scheduler-systems/gal-code-console-core/billing.js"
import { Actor } from "@scheduler-systems/gal-code-console-core/actor.js"
import { WorkspaceTable } from "@scheduler-systems/gal-code-console-core/schema/workspace.sql.js"
import { ZenData } from "@scheduler-systems/gal-code-console-core/model.js"
import { Subscription } from "@scheduler-systems/gal-code-console-core/subscription.js"
import { BlackData } from "@scheduler-systems/gal-code-console-core/black.js"
import { UserTable } from "@scheduler-systems/gal-code-console-core/schema/user.sql.js"
import { ModelTable } from "@scheduler-systems/gal-code-console-core/schema/model.sql.js"
import { ProviderTable } from "@scheduler-systems/gal-code-console-core/schema/provider.sql.js"
...
import { LiteData } from "@scheduler-systems/gal-code-console-core/lite.js"
import { Resource } from "@scheduler-systems/gal-code-console-resource"
...
export async function handler(
  ...
  function getModelContextLimit(modelId: string): number {
    const baseModel = modelId.split(":")[0]?.split("-")[0] ?? modelId
    for (const [key, limit] of Object.entries(MODEL_CONTEXT_LIMITS)) {
      if (modelId === key || baseModel === key) return limit
      if (modelId.startsWith(key)) return limit
      if (key.startsWith(baseModel)) return limit
    }
    return 200000
  }
  ...
  try {
    ...
    const sessionId = input.request.headers.get("x-gal-code-session") ?? ""
    const requestId = input.request.headers.get("x-gal-code-request") ?? ""
    const projectId = input.request.headers.get("x-gal-code-project") ?? ""
    const ocClient = input.request.headers.get("x-gal-code-client") ?? ""
    logger.metric({
      is_stream: isStream,
      session: sessionId,
      request: requestId,
    ...
    const estimatedTokens = Token.estimateRequest({
      messages: body.messages || [],
      tools: body.tools,
      max_tokens: body.max_tokens,
    })
    if (estimatedTokens > contextLimit) {
      const reductionNeeded = estimatedTokens - contextLimit
      throw new ContextLimitError(
        Token.buildLimitErrorMessage(estimatedTokens, contextLimit, model),
        estimatedTokens,
        contextLimit,
        reductionNeeded,
      )
    ...
    const retriableRequest = async (retry: RetryOptions = { excludeProviders: [], retryCount: 0 }) => {
      ...
      const res = await fetchWith429Retry(reqUrl, {
        ...
        headers: (() => {
          ...
          headers.delete("x-gal-code-request")
          headers.delete("x-gal-code-session")
          headers.delete("x-gal-code-project")
          headers.delete("x-gal-code-client")
          return headers
        })(),
        body: reqBody,
      })
    ...
    if (error instanceof ContextLimitError) {
      logger.metric({
        "tokens.estimated": error.estimatedTokens,
        "tokens.context_limit": error.maxContextTokens,
        "tokens.reduction_needed": error.reductionNeeded,
      })
      return new Response(
        JSON.stringify({
          type: "error",
          error: {
            ...
            details: {
              estimated_tokens: error.estimatedTokens,
              max_context_tokens: error.maxContextTokens,
              reduction_needed: error.reductionNeeded,
            },
          },
        }),
        { status: 400 },
  ...
  function validateBilling(authInfo: AuthInfo, modelInfo: ModelInfo): BillingSource {
    ...
    const billingUrl = `https://gal.run/workspace/${authInfo.workspaceID}/billing`
    const membersUrl = `https://gal.run/workspace/${authInfo.workspaceID}/members`
