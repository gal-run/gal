...
import { createGalCode, type ToolPart } from "@scheduler-systems/gal-code-sdk"
...
console.log("🚀 Starting gal-code server...")
const galCode = await createGalCode({
  port: 0,
})
console.log("✅ GAL Code server ready")

const sessions = new Map<string, { client: any; server: any; sessionId: string; channel: string; thread: string }>()
;(async () => {
  const events = await galCode.client.event.subscribe()
...
app.message(async ({ message, say }) => {
  ...
  if (!session) {
    console.log("🆕 Creating new gal-code session...")
    const { client, server } = galCode
    ...
    console.log("✅ Created gal-code session:", createResult.data.id)
  ...
  console.log("📝 Sending to gal-code:", message.text)
  const result = await session.client.session.prompt({
    path: { id: session.sessionId },
    body: { parts: [{ type: "text", text: message.text }] },
  })

  console.log("📤 GAL Code response:", JSON.stringify(result, null, 2))
