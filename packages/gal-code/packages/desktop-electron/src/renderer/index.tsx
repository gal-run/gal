...
import {
  ...
  ServerConnection,
  useCommand,
} from "@scheduler-systems/gal-code-app"
...
import "./styles.css"
import { useTheme } from "@scheduler-systems/gal-code-ui/theme"
...
const deepLinkEvent = "gal-code:deep-link"

const emitDeepLinks = (urls: string[]) => {
  if (urls.length === 0) return
  window.__GAL_CODE__ ??= {}
  const pending = window.__GAL_CODE__.deepLinks ?? []
  window.__GAL_CODE__.deepLinks = [...pending, ...urls]
  window.dispatchEvent(new CustomEvent(deepLinkEvent, { detail: { urls } }))
}

const listenForDeepLinks = () => {
  const startUrls = window.__GAL_CODE__?.deepLinks ?? []
...
const createPlatform = (): Platform => {
  ...
  const wslHome = async () => {
    if (os !== "windows" || !window.__GAL_CODE__?.wsl) return undefined
    return window.api.wslPath("~", "windows").catch(() => undefined)
  }

  const handleWslPicker = async <T extends string | string[]>(result: T | null): Promise<T | null> => {
    if (!result || !window.__GAL_CODE__?.wsl) return result
  ...
  return {
    ...
    notify: async (title, description, href) => {
      ...
      const notification = new Notification(title, {
        body: description ?? "",
        icon: "https://gal.run/favicon-96x96-v3.png",
      })
    ...
    getWslEnabled: async () => {
      ...
      return window.__GAL_CODE__!.wsl ?? false
    },
...
render(() => {
  ...
  const loadLocale = async () => {
    const current = await platform.storage?.("gal-code.global.dat").getItem("language")
