...
function tauriAppId() {
  return app.isPackaged ? TAURI_APP_IDS[CHANNEL] : "ai.gal/code.desktop.dev"
}

// Migrate a single Tauri .dat file into the corresponding electron-store.
// `gal-code.settings.dat` is special: it maps to the `gal-code.settings` store
...
// renderer already passes via IPC (e.g. `"default.dat"`, `"gal-code.global.dat"`).
function migrateFile(datPath: string, filename: string) {
  ...
  // gal-code.settings.dat → the electron settings store ("gal-code.settings").
  // All other .dat files keep their full filename as the store name so they match
  // what the renderer passes via IPC (e.g. "default.dat", "gal-code.global.dat").
  const storeName = filename === "gal-code.settings.dat" ? "gal-code.settings" : filename
