...
- Author package entrypoint is `@gal-code-ai/plugin/tui`.
...
```json
{
  "$schema": "https://gal.run/tui.json",
  "theme": "smoke-theme",
  "plugin": ["@acme/gal-code-plugin@1.2.3", ["./plugins/demo.tsx", { "label": "demo" }]],
  "plugin_enabled": {
    "acme.demo": false
  }
}
...
- Import types from `@gal-code-ai/plugin/tui`.
- `@gal-code-ai/plugin` exports `./tui` and declares optional peer deps on `@opentui/core` and `@opentui/solid`.

Minimal module shape:

```tsx
/** @jsxImportSource @opentui/solid */
import type { TuiPlugin, TuiPluginModule } from "@gal-code-ai/plugin/tui"
...
- The value is a semver range checked against the running GAL Code version.
- If the range is not satisfied, the plugin is skipped with a warning and a session error.
- If `engines.gal-code` is absent, no check is performed (backward compatible).
...
- `gal-code plugin <module>` and TUI install both run install → manifest read → config patch.
- Alias: `gal-code plug <module>`.
- `-g` / `--global` writes into the global config dir.
- Local installs resolve target dir inside `patchPluginConfig`.
- For local scope, path is `<worktree>/.gal-code` only when VCS is git and `worktree !== "/"`; otherwise `<directory>/.gal-code`.
- Root-worktree fallback (`worktree === "/"` uses `<directory>/.gal-code`) is covered by regression tests.
...
When `plugin` entries exist in a writable `.gal-code` dir or `OPENCODE_CONFIG_DIR`, GAL Code installs `@gal-code-ai/plugin` into that dir and writes:
...
That is what makes local config-scoped plugins able to import `@gal-code-ai/plugin/tui`.
...
- Local plugins persist installed themes under the local `.gal-code/themes` area near the plugin config source.
...
- Local smoke plugin: `.gal-code/plugins/tui-smoke.tsx`
- Local vim plugin: `.gal-code/plugins/tui-vim.tsx`
- Local smoke config: `.gal-code/tui.json`
- Local smoke theme: `.gal-code/plugins/smoke-theme.json`
