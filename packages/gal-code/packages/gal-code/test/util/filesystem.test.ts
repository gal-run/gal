...
describe("filesystem", () => {
  ...
  describe("findUp()", () => {
    ...
    test("rootFirst preserves json then jsonc order per directory", async () => {
      ...
      await fs.writeFile(path.join(tmp.path, "gal-code.json"), "{}", "utf-8")
      await fs.writeFile(path.join(tmp.path, "gal-code.jsonc"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "gal-code.json"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "gal-code.jsonc"), "{}", "utf-8")

      const result = await Filesystem.findUp(["gal-code.json", "gal-code.jsonc"], nested, tmp.path, {
        rootFirst: true,
      })

      expect(result).toEqual([
        path.join(tmp.path, "gal-code.json"),
        path.join(tmp.path, "gal-code.jsonc"),
        path.join(project, "gal-code.json"),
        path.join(project, "gal-code.jsonc"),
      ])
    })
  })
