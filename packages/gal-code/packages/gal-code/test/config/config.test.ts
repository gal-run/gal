...
const managedConfigDir = process.env.GAL_CODE_TEST_MANAGED_CONFIG_DIR!
...
async function writeManagedSettings(settings: object, filename = "gal-code.json") {
  await fs.mkdir(managedConfigDir, { recursive: true })
  await Filesystem.write(path.join(managedConfigDir, filename), JSON.stringify(settings))
}

async function writeConfig(dir: string, config: object, name = "gal-code.json") {
...
test("ignores legacy tui keys in gal-code config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await writeConfig(dir, {
        $schema: "https://gal.run/config.json",
  ...
  try {
    ...
    await Instance.provide({
      ...
      fn: async () => {
        ...
        const content = await Filesystem.readText(path.join(tmp.path, "gal-code.json"))
...
test("resolves env templates in account config with account token", async () => {
  const originalControlToken = process.env["GAL_CODE_CONSOLE_TOKEN"]
  ...
  const fakeAccount = Layer.mock(Account.Service)({
    ...
    config: () =>
      Effect.succeed(
        Option.some({
          provider: { "gal-code": { options: { apiKey: "{env:GAL_CODE_CONSOLE_TOKEN}" } } },
        }),
      ),
    ...
    await provideTmpdirInstance(() =>
      Config.Service.use((svc) =>
        Effect.gen(function* () {
          ...
          expect(config.provider?.["gal-code"]?.options?.apiKey).toBe("st_test_token")
        }),
      ),
    ).pipe(Effect.scoped, Effect.provide(layer), Effect.runPromise)
  } finally {
    if (originalControlToken !== undefined) {
      process.env["GAL_CODE_CONSOLE_TOKEN"] = originalControlToken
    } else {
...
test("does not trust local GAL-T entitlement markers", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await writeConfig(dir, {
        $schema: "https://gal.run/config.json",
  ...
  await Instance.provide({
    ...
    fn: async () => {
      const config = await Config.get()
      const access = await Config.getGALTAccess()
      expect(config.galt?.enabled).toBe(true)
      expect(access.entitlements).toEqual([])
    },
  })
})

test("trusts GAL-T cyber entitlement from account config", async () => {
  const fakeAccount = Layer.mock(Account.Service)({
    active: () =>
      Effect.succeed(
        Option.some({
  ...
  await provideTmpdirInstance(() =>
    Config.Service.use((svc) =>
      Effect.gen(function* () {
        const config = yield* svc.get()
        const access = yield* svc.getGALTAccess()
        expect(config.galt?.enabled).toBe(true)
        expect(access.entitlements).toEqual(["cyber"])
        expect(access.sources).toEqual(["https://control.example.com/api/config"])
      }),
    ),
  ).pipe(Effect.scoped, Effect.provide(layer), Effect.runPromise)
})
...
test("throws error for invalid JSON", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await Filesystem.write(path.join(dir, "gal-code.json"), "{ invalid json }")
    },
  })
  await Instance.provide({
    directory: tmp.path,
...
test("loads config from .gal/code directory", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const galCodeDir = path.join(dir, ".gal/code")
      await fs.mkdir(galCodeDir, { recursive: true })
      const agentDir = path.join(galCodeDir, "agent")
...
test("loads agents from .gal/code/agents (plural)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const galCodeDir = path.join(dir, ".gal/code")
      await fs.mkdir(galCodeDir, { recursive: true })

      const agentsDir = path.join(galCodeDir, "agents")
...
test("loads commands from .gal/code/command (singular)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const galCodeDir = path.join(dir, ".gal/code")
      await fs.mkdir(galCodeDir, { recursive: true })

      const commandDir = path.join(galCodeDir, "command")
...
test("loads commands from .gal/code/commands (plural)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const galCodeDir = path.join(dir, ".gal/code")
      await fs.mkdir(galCodeDir, { recursive: true })

      const commandsDir = path.join(galCodeDir, "commands")
...
test("does not try to install dependencies in read-only GAL_CODE_CONFIG_DIR", async () => {
  if (process.platform === "win32") return
  ...
  const prev = process.env.GAL_CODE_CONFIG_DIR
  process.env.GAL_CODE_CONFIG_DIR = tmp.extra
  ...
  try {
    ...
    if (prev === undefined) delete process.env.GAL_CODE_CONFIG_DIR
    else process.env.GAL_CODE_CONFIG_DIR = prev
  }
})

test("installs dependencies in writable GAL_CODE_CONFIG_DIR", async () => {
  await using tmp = await tmpdir<string>({
    init: async (dir) => {
  ...
  const install = spyOn(Npm, "install").mockImplementation(async (dir: string) => {
    const mod = path.join(dir, "node_modules", "@gal-code-ai", "plugin")
    await fs.mkdir(mod, { recursive: true })
    await Filesystem.write(
      path.join(mod, "package.json"),
      JSON.stringify({ name: "@scheduler-systems/gal-code-plugin", version: "1.0.0" }),
    )
  })
...
test("dedupes concurrent config dependency installs for the same dir", async () => {
  ...
  const run = spyOn(Npm, "install").mockImplementation(async (d: string) => {
    ...
    const mod = path.join(d, "node_modules", "@gal-code-ai", "plugin")
    await fs.mkdir(mod, { recursive: true })
    await Filesystem.write(
      path.join(mod, "package.json"),
      JSON.stringify({ name: "@scheduler-systems/gal-code-plugin", version: "1.0.0" }),
...
test("serializes config dependency installs across dirs", async () => {
  ...
  const run = spyOn(Npm, "install").mockImplementation(async (dir: string) => {
    ...
    const mod = path.join(cwd, "node_modules", "@gal-code-ai", "plugin")
    await fs.mkdir(mod, { recursive: true })
    await Filesystem.write(
      path.join(mod, "package.json"),
      JSON.stringify({ name: "@scheduler-systems/gal-code-plugin", version: "1.0.0" }),
...
test("resolves scoped npm plugins in config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      await Filesystem.write(
        path.join(dir, "gal-code.json"),
        JSON.stringify({ $schema: "https://gal.run/config.json", plugin: ["@scope/plugin"] }, null, 2),
      )
    },
  })
...
test("merges plugin arrays from global and local configs", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      // Create a nested project structure with local .gal/code config
      const projectDir = path.join(dir, "project")
      const galCodeDir = path.join(projectDir, ".gal/code")
      await fs.mkdir(galCodeDir, { recursive: true })
      ...
      // Local .gal/code config with different plugins
      await Filesystem.write(
        path.join(galCodeDir, "gal-code.json"),
        JSON.stringify({
          $schema: "https://gal.run/config.json",
...
test("deduplicates duplicate plugins from global and local configs", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      // Local .gal/code config with some overlapping plugins
      await Filesystem.write(
        path.join(galCodeDir, "gal-code.json"),
        JSON.stringify({
          $schema: "https://gal.run/config.json",
...
test("keeps plugin origins aligned with merged plugin list", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      const local = path.join(project, ".gal/code")
...
// Note: preload.ts sets GAL_CODE_TEST_MANAGED_CONFIG which Global.Path.managedConfig uses
...
test("local .gal/code config can override MCP from project config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      // Local .gal/code directory config enables it
      const galCodeDir = path.join(dir, ".gal/code")
      await fs.mkdir(galCodeDir, { recursive: true })
      await Filesystem.write(
        path.join(galCodeDir, "gal-code.json"),
...
test("project config overrides remote well-known config", async () => {
  ...
  globalThis.fetch = mock((url: string | URL | Request) => {
    ...
    if (urlStr.includes(".well-known/gal-code")) {
      fetchedUrl = urlStr
    ...
    await provideTmpdirInstance(
      () =>
        Config.Service.use((svc) =>
          Effect.gen(function* () {
            const config = yield* svc.get()
            expect(fetchedUrl).toBe("https://example.com/.well-known/gal-code")
...
describe("resolvePluginSpec", () => {
  test("keeps package specs unchanged", async () => {
    await using tmp = await tmpdir()
    const file = path.join(tmp.path, "gal-code.json")
    expect(await Config.resolvePluginSpec("oh-my-gal-code@2.4.3", file)).toBe("oh-my-gal-code@2.4.3")
...
describe("deduplicatePluginOrigins", () => {
  ...
  test("keeps path plugins separate from package plugins", () => {
    const plugins = ["oh-my-gal-code@2.4.3", "file:///project/.gal/code/plugin/oh-my-gal-code.js"]
  ...
  test("deduplicates direct path plugins by exact spec", () => {
    const plugins = ["file:///project/.gal/code/plugin/demo.ts", "file:///project/.gal/code/plugin/demo.ts"]

    const result = dedupe(plugins)

    expect(result).toEqual(["file:///project/.gal/code/plugin/demo.ts"])
  })
  ...
  test("loads auto-discovered local plugins as file urls", async () => {
    await using tmp = await tmpdir({
      init: async (dir) => {
        const projectDir = path.join(dir, "project")
        const galCodeDir = path.join(projectDir, ".gal/code")
        const pluginDir = path.join(galCodeDir, "plugin")
        await fs.mkdir(pluginDir, { recursive: true })
...
describe("GAL_CODE_DISABLE_PROJECT_CONFIG", () => {
  test("skips project config files when flag is set", async () => {
    const originalEnv = process.env["GAL_CODE_DISABLE_PROJECT_CONFIG"]
    process.env["GAL_CODE_DISABLE_PROJECT_CONFIG"] = "true"
    ...
    try {
      ...
      if (originalEnv === undefined) {
        delete process.env["GAL_CODE_DISABLE_PROJECT_CONFIG"]
      } else {
        process.env["GAL_CODE_DISABLE_PROJECT_CONFIG"] = originalEnv
      }
    }
  })

  test("skips project .gal/code/ directories when flag is set", async () => {
    const originalEnv = process.env["GAL_CODE_DISABLE_PROJECT_CONFIG"]
    process.env["GAL_CODE_DISABLE_PROJECT_CONFIG"] = "true"
    ...
    try {
      await using tmp = await tmpdir({
        init: async (dir) => {
          // Create a .gal/code directory with a command
          const galCodeDir = path.join(dir, ".gal/code", "command")
          await fs.mkdir(galCodeDir, { recursive: true })
          await Filesystem.write(path.join(galCodeDir, "test-cmd.md"), "# Test Command\nThis is a test command.")
        },
      })
      await Instance.provide({
        directory: tmp.path,
        fn: async () => {
          ...
          // Project .gal/code should NOT be in directories list
          const hasProjectGalCode = directories.some((d) => d.startsWith(tmp.path))
          expect(hasProjectGalCode).toBe(false)
        },
      })
    } finally {
      if (originalEnv === undefined) {
  ...
  test("skips relative instructions with warning when flag is set but no config dir", async () => {
    const originalDisable = process.env["GAL_CODE_DISABLE_PROJECT_CONFIG"]
    const originalConfigDir = process.env["GAL_CODE_CONFIG_DIR"]

    try {
      // Ensure no config dir is set
      delete process.env["GAL_CODE_CONFIG_DIR"]
      process.env["GAL_CODE_DISABLE_PROJECT_CONFIG"] = "true"
      ...
      if (originalDisable === undefined) {
        ...
        process.env["GAL_CODE_DISABLE_PROJECT_CONFIG"] = originalDisable
      }
      if (originalConfigDir === undefined) {
        delete process.env["GAL_CODE_CONFIG_DIR"]
      } else {
        process.env["GAL_CODE_CONFIG_DIR"] = originalConfigDir
      }
    }
  })

  test("GAL_CODE_CONFIG_DIR still works when flag is set", async () => {
    const originalDisable = process.env["GAL_CODE_DISABLE_PROJECT_CONFIG"]
    const originalConfigDir = process.env["GAL_CODE_CONFIG_DIR"]
    ...
    try {
      ...
      process.env["GAL_CODE_DISABLE_PROJECT_CONFIG"] = "true"
      process.env["GAL_CODE_CONFIG_DIR"] = configDirTmp.path

      await Instance.provide({
        directory: projectTmp.path,
        fn: async () => {
          const config = await Config.get()
          // Should load from GAL_CODE_CONFIG_DIR, not project
...
describe("GAL_CODE_CONFIG_CONTENT token substitution", () => {
  test("substitutes {env:} tokens in GAL_CODE_CONFIG_CONTENT", async () => {
    const originalEnv = process.env["GAL_CODE_CONFIG_CONTENT"]
    const originalTestVar = process.env["TEST_CONFIG_VAR"]
    process.env["TEST_CONFIG_VAR"] = "test_api_key_12345"
    process.env["GAL_CODE_CONFIG_CONTENT"] = JSON.stringify({
      $schema: "https://gal.run/config.json",
    ...
    try {
      ...
      if (originalEnv !== undefined) {
        process.env["GAL_CODE_CONFIG_CONTENT"] = originalEnv
      } else {
        delete process.env["GAL_CODE_CONFIG_CONTENT"]
      }
  ...
  test("substitutes {file:} tokens in GAL_CODE_CONFIG_CONTENT", async () => {
    const originalEnv = process.env["GAL_CODE_CONFIG_CONTENT"]
...
test("parseManagedPlist strips MDM metadata keys", async () => {
  const config = await Config.parseManagedPlist(
    JSON.stringify({
      PayloadDisplayName: "GAL Code Managed",
      PayloadIdentifier: "ai.gal/code.managed.test",
      PayloadType: "ai.gal/code.managed",
...
test("parseManagedPlist handles empty config", async () => {
  const config = await Config.parseManagedPlist(
    JSON.stringify({ $schema: "https://gal.run/config.json" }),
    "test:mobileconfig",
  )
  expect(config.$schema).toBe("https://gal.run/config.json")
})
