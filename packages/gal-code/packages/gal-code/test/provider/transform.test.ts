...
describe("ProviderTransform.options - zai/zhipuai thinking", () => {
  ...
  test("glm-vertex with family 'glm' should set thinking cfg", () => {
    const glmModel = {
      ...createModel("glm-vertex"),
      id: "glm-vertex/glm-4.7",
      api: {
        id: "zai-org/glm-4.7-maas",
        url: "https://api.gal.run/api/gal-code/v1",
        npm: "@ai-sdk/openai-compatible",
      },
      family: "glm",
      name: "GLM-4.7",
...
describe("ProviderTransform.message - strip openai metadata when store=false", () => {
  ...
  test("preserves metadata using providerID key when store is false", () => {
    ...
    const result = ProviderTransform.message(msgs, galCodeModel, { store: false }) as any[]

    expect(result[0].content[0].providerOptions?.["gal-code"]?.itemId).toBe("msg_123")
    expect(result[0].content[0].providerOptions?.["gal-code"]?.otherOption).toBe("value")
  })
  ...
  test("preserves itemId across all providerOptions keys", () => {
        ...
        content: [
          {
            ...
            providerOptions: {
              ...
              "gal-code": { itemId: "msg_gal-code_part" },
    ...
    expect(result[0].providerOptions?.["gal-code"]?.itemId).toBe("msg_gal-code")
    expect(result[0].providerOptions?.extra?.itemId).toBe("msg_extra")
    expect(result[0].content[0].providerOptions?.openai?.itemId).toBe("msg_openai_part")
    expect(result[0].content[0].providerOptions?.["gal-code"]?.itemId).toBe("msg_gal-code_part")
...
describe("ProviderTransform.variants", () => {
  ...
  test("deepseek returns high/max thinking variants", () => {
    const model = createMockModel({
