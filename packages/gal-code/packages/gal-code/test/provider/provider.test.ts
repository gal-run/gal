...
import { installProviderFixtures } from "./provider-fixtures"

installProviderFixtures()

function paid(providers: Awaited<ReturnType<typeof Provider.list>>) {
  const item = providers[ProviderID.make("gal-code")]
    ...
    init: async (dir) => {
      await Bun.write(
        ...
        JSON.stringify({
          ...
          provider: {
            "gal-code": {
              blacklist: ["zai-org/glm-4.7-maas", "qwen3.5-35b-a3b-fp8", "deepseek-v4-pro"],
            },
          },
        }),
      )
...
test("provider loaded from secret manager", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await Bun.write(
        path.join(dir, "gal-code.json"),
  ...
  const load = spyOn(Secret, "load").mockImplementation(async (name) =>
    name === "ANTHROPIC_API_KEY" ? "test-secret-key" : undefined,
  )
  ...
  try {
    await Instance.provide({
      ...
      fn: async () => {
        const providers = await Provider.list()
        expect(load).toHaveBeenCalledWith("ANTHROPIC_API_KEY")
        expect(providers[ProviderID.anthropic]).toBeDefined()
        expect(providers[ProviderID.anthropic].source).toBe("secret")
        expect(providers[ProviderID.anthropic].key).toBe("test-secret-key")
      },
    })
  } finally {
    load.mockRestore()
...
test("env variable wins over secret manager", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await Bun.write(
        path.join(dir, "gal-code.json"),
  ...
  const load = spyOn(Secret, "load").mockResolvedValue("test-secret-key")
  ...
  try {
    await Instance.provide({
      ...
      init: async () => {
        Env.set("GAL_CODE_SECRET_PROJECT", "gal-run")
        Env.set("ANTHROPIC_API_KEY", "test-env-key")
      },
      fn: async () => {
        const providers = await Provider.list()
        expect(load.mock.calls.some((call) => call[0] === "ANTHROPIC_API_KEY")).toBe(false)
        expect(providers[ProviderID.anthropic]).toBeDefined()
        expect(providers[ProviderID.anthropic].source).toBe("env")
        expect(providers[ProviderID.anthropic].key).toBe("test-env-key")
      },
    })
  } finally {
    load.mockRestore()
...
test("gal gateway glm-4.7 provider loads from env and config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await Bun.write(
        path.join(dir, "gal-code.json"),
  ...
  await Instance.provide({
    directory: tmp.path,
    init: async () => {
      Env.set("GAL_AUTH_TOKEN", "gal-auth-token")
    },
    fn: async () => {
      const providers = await Provider.list()
      const provider = providers[ProviderID.make("glm-vertex")]
      expect(provider).toBeDefined()
      expect(provider.source).toBe("config")
      expect(provider.key).toBe("gal-auth-token")
      expect(provider.options.baseURL).toBe("https://api.gal.run/api/gal-code/v1")
      expect(provider.models["glm-4.7"]).toBeDefined()
      expect(provider.models["glm-4.7"].api.id).toBe("zai-org/glm-4.7-maas")
      expect(provider.models["glm-4.7"].api.npm).toBe("@ai-sdk/openai-compatible")
    },
  })
})

test("deepseek provider loads from direct API env", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await Bun.write(
        path.join(dir, "gal-code.json"),
  ...
  await Instance.provide({
    directory: tmp.path,
    init: async () => {
      Env.set("DEEPSEEK_API_KEY", "test-key")
    },
    fn: async () => {
      const provider = (await Provider.list())[ProviderID.make("deepseek")]
      expect(provider).toBeDefined()
      expect(provider.source).toBe("env")
      expect(provider.key).toBe("test-key")
      expect(provider.options.apiKey).toBe("test-key")
      expect(provider.options.baseURL).toBe("https://api.deepseek.com")
      expect(provider.options.timeout).toBe(90_000)
      expect(provider.options.chunkTimeout).toBe(30_000)
      expect(provider.models["deepseek-v4-pro"]).toBeDefined()
      expect(provider.models["deepseek-v4-flash"]).toBeDefined()
      expect(provider.models["deepseek-v4-pro"].api.npm).toBe("@ai-sdk/openai-compatible")
      expect(provider.models["deepseek-v4-pro"].api.url).toBe("https://api.deepseek.com")
      expect(provider.models["deepseek-v4-pro"].family).toBe("deepseek")
      expect(provider.models["deepseek-v4-pro"].capabilities.interleaved).toEqual({
        field: "reasoning_content",
      })
      expect(provider.models["deepseek-v4-pro"].variants).toEqual({
...
test("deepseek provider accepts anthropic env when base url points at deepseek", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await Bun.write(
        path.join(dir, "gal-code.json"),
  ...
  await Instance.provide({
    ...
    init: async () => {
      ...
      Env.set("ANTHROPIC_BASE_URL", "https://api.deepseek.com/anthropic")
    },
    fn: async () => {
      const provider = (await Provider.list())[ProviderID.make("deepseek")]
      expect(provider).toBeDefined()
...
test("defaultModel prefers top backend-ranked gal-code model", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await Bun.write(
        path.join(dir, "gal-code.json"),
  ...
  await Instance.provide({
    directory: tmp.path,
    init: async () => {
      Env.set("GAL_AUTH_TOKEN", "test-auth-token")
    },
    fn: async () => {
      const model = await Provider.defaultModel()
      expect(String(model.providerID)).toBe("gal-code")
      expect(String(model.modelID)).toBe("deepseek-v4-pro")

      const provider = await Provider.getProvider(ProviderID.make("gal-code"))
      expect(provider.models["deepseek-v4-pro"]).toBeDefined()
      expect(provider.models["qwen3.5-35b-a3b-fp8"]).toBeDefined()
      expect(provider.models["zai-org/glm-4.7-maas"]).toBeDefined()
    },
  })
})
...
test("provider.sort prioritizes preferred models", () => {
  const models = [
    ...
    { id: "moonshotai/Kimi-K2.6", name: "Kimi" },
  ...
  expect(sorted[sorted.length - 1].id).toBe("random-model")
})
        ...
        JSON.stringify({
          ...
          provider: {
            "cloudflare-ai-gateway": {
              options: {
                metadata: { invoked_by: "test", project: "gal-code" },
              },
            },
          },
        }),
...
test(
  ...
  async () => {
    await using tmp = await tmpdir({
      init: async (dir) => {
        const root = path.join(dir, ".gal/code", "plugin")
...
test("gal-code loader keeps the gateway-backed models available without a local apiKey", async () => {
...
test("gal-code loader keeps paid models when auth exists", async () => {
  await using base = await tmpdir({
    init: async (dir) => {
      await Bun.write(
        path.join(dir, "gal-code.json"),
