...
import { resolvePluginProviders, resolveProviderSources } from "../../src/cli/cmd/providers"
...
describe("resolveProviderSources", () => {
  test("lists active environment variables from the catalog", () => {
    const result = resolveProviderSources({
      database: {
        deepseek: {
          name: "DeepSeek",
  ...
  test("lists secret-backed providers from the live provider set", () => {
    const result = resolveProviderSources({
      database: {
        anthropic: {
          name: "Anthropic",
