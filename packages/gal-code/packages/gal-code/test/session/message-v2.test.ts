...
describe("session.message-v2.fromError", () => {
  ...
  test("serializes response error codes", () => {
    const cases = [
      ...
      },
      {
        ...
        message: "Runtime funding is unavailable. Provider credit is exhausted for this work right now.",
        metadata: {
          reasonCode: "provider_credit_insufficient",
        },
      },
    ]
  ...
  test("serializes runtime funding failures from APICallError bodies", () => {
    const error = new APICallError({
      message: "Payment Required",
      url: "https://api.gal.run",
      requestBodyValues: {},
      statusCode: 402,
      responseHeaders: { "content-type": "application/json" },
      responseBody: JSON.stringify({
        code: "PROVIDER_CREDIT_INSUFFICIENT",
        message: "GAL Gateway upstream 402 Payment Required: Insufficient Balance",
        runtimeFunding: {
          reasonCode: "provider_credit_insufficient",
        },
      }),
  ...
  test("classifies plain 402 insufficient balance as explicit runtime funding failure", () => {
    const error = new APICallError({
      message: "GAL Gateway upstream 402 Payment Required: Insufficient Balance",
      url: "https://api.gal.run",
      requestBodyValues: {},
  ...
  test("detects context overflow from GLM invalid argument bodies", () => {
    const error = new APICallError({
      message: "Bad Request",
      url: "https://example.com",
      requestBodyValues: {},
      ...
      responseBody: JSON.stringify({
        error: {
          code: 400,
          message:
            "Requested token count exceeds the model's maximum context length of 202752 tokens. You requested a total of 211266 tokens: 179266 tokens from the input messages and 32000 tokens for the completion. Please reduce the number of tokens in the input messages or the completion to fit within the limit.",
  ...
  test("classifies ZlibError from fetch as retryable APIError", () => {
    const zlibError = new Error(
      'ZlibError fetching "https://gal-code.cloudflare.dev/anthropic/messages". For more information, pass `verbose: true` in the second argument to fetch()',
    )
