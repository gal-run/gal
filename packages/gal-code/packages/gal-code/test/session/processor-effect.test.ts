...
it.live("session.processor effect tests normalize <none> in retry status message (#6580)", () =>
  provideTmpdirServer(
    ({ dir, llm }) =>
      Effect.gen(function* () {
        const { processors, session, provider } = yield* boot()
        ...
        const off = yield* bus.subscribeCallback(SessionStatus.Event.Status, (evt) => {
          ...
          if (evt.properties.status.type === "retry") {
            retryMessage = evt.properties.status.message
          }
        })
        const handle = yield* processors.create({
          assistantMessage: msg,
        ...
        expect(retryMessage).not.toBe("<none>")
        expect(retryMessage).toBe('Service Unavailable: "<none>"')
      }),
    { git: true, config: (url) => providerCfg(url) },
  ),
)
