...
describe("session.llm.stream", () => {
    ...
    await using tmp = await tmpdir({
      init: async (dir) => {
        await Bun.write(
          ...
          JSON.stringify(providerConfig(fixture, { apiKey: "test-key", baseURL: `${server.url.origin}/v1` })),
        )
      },
    })
  ...
  test("sends responses API payload for OpenAI models", async () => {
    ...
    const source = await loadProviderFixture("openai", "gpt-5.2")
    ...
    await using tmp = await tmpdir({
      init: async (dir) => {
        await Bun.write(
          path.join(dir, "gal-code.json"),
          JSON.stringify(providerConfig(source, { apiKey: "test-openai-key", baseURL: `${server.url.origin}/v1` })),
        )
      },
    })
  ...
  test("sends messages API payload for Anthropic Compatible models", async () => {
    ...
    await using tmp = await tmpdir({
      init: async (dir) => {
        await Bun.write(
          path.join(dir, "gal-code.json"),
          JSON.stringify(providerConfig(fixture, { apiKey: "test-anthropic-key", baseURL: `${server.url.origin}/v1` })),
        )
      },
    })
  ...
  test("sends Google API payload for Gemini models", async () => {
    ...
    await using tmp = await tmpdir({
      init: async (dir) => {
        await Bun.write(
          path.join(dir, "gal-code.json"),
          JSON.stringify(providerConfig(fixture, { apiKey: "test-google-key", baseURL: `${server.url.origin}/v1beta` })),
        )
      },
    })
