...
test("swarm agent is mcp-only, read-only, and denies edits", async () => {
  await using tmp = await tmpdir()
  await Instance.provide({
    directory: tmp.path,
    fn: async () => {
      const swarm = await Agent.get("swarm")
      expect(swarm).toBeDefined()
      expect(swarm?.mode).toBe("primary")
      expect(evalPerm(swarm, "gal_swarm_run")).toBe("allow")
      expect(evalPerm(swarm, "gal_swarm_list_runs")).toBe("allow")
      expect(evalPerm(swarm, "gal_swarm_status")).toBe("allow")
      expect(evalPerm(swarm, "gal_swarm_calibrate")).toBe("allow")
      expect(evalPerm(swarm, "gal_swarm_observe_capacity")).toBe("allow")
      expect(evalPerm(swarm, "read")).toBe("allow")
      expect(evalPerm(swarm, "grep")).toBe("allow")
      expect(evalPerm(swarm, "list")).toBe("allow")
      expect(evalPerm(swarm, "bash")).toBe("deny")
      expect(evalPerm(swarm, "webfetch")).toBe("deny")
      expect(evalPerm(swarm, "gal_dispatch_agent")).toBe("deny")
      expect(evalPerm(swarm, "gal_enqueue_work_items")).toBe("deny")
      expect(evalPerm(swarm, "edit")).toBe("deny")
    },
  })
})

test("swarm agent allows enterprise sibling paths and denies external directories outside the workspace root", async () => {
  await using tmp = await tmpdir()
  ...
  await Bun.write(path.join(repo, "README.md"), "repo\n")
  await Bun.write(path.join(sibling, "notes.txt"), "evidence\n")
  ...
  await $`git config user.email "test@gal.run"`.cwd(repo).quiet()
  await $`git config user.name "Test"`.cwd(repo).quiet()
  await $`git add README.md`.cwd(repo).quiet()
  await $`git commit -m "root commit"`.cwd(repo).quiet()
  ...
  await Instance.provide({
    ...
    fn: async () => {
      const swarm = await Agent.get("swarm")
      expect(swarm).toBeDefined()
      expect(Permission.evaluate("external_directory", sibling, swarm!.permission).action).toBe("allow")
      expect(Permission.evaluate("external_directory", "/tmp/not-enterprise", swarm!.permission).action).toBe("deny")
    },
  })
})

test("plan agent denies edits except .gal/code/plans/*", async () => {
  await using tmp = await tmpdir()
  await Instance.provide({
    directory: tmp.path,
    fn: async () => {
      ...
      expect(Permission.evaluate("edit", ".gal/code/plans/foo.md", plan!.permission).action).toBe("allow")
    },
  })
})
...
test("skill directories are allowed for external_directory", async () => {
  await using tmp = await tmpdir({
    git: true,
    init: async (dir) => {
      const skillDir = path.join(dir, ".gal/code", "skill", "perm-skill")
      await Bun.write(
...
`,
  ...
  const home = process.env.GAL_CODE_TEST_HOME
  process.env.GAL_CODE_TEST_HOME = tmp.path
  ...
  try {
    await Instance.provide({
      ...
      fn: async () => {
        const build = await Agent.get("build")
        const skillDir = path.join(tmp.path, ".gal/code", "skill", "perm-skill")
        const target = path.join(skillDir, "reference", "notes.md")
        expect(Permission.evaluate("external_directory", target, build!.permission).action).toBe("allow")
      },
    })
  } finally {
    process.env.GAL_CODE_TEST_HOME = home
  }
})
...
test("defaultAgent throws when all primary agents are disabled", async () => {
  ...
  await Instance.provide({
    directory: tmp.path,
    fn: async () => {
      // build, plan, and swarm are disabled, no primary-capable agents remain
