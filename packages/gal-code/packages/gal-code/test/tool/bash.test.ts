...
import { BackgroundWaitTool, BashKillTool, BashOutputTool } from "../../src/tool/background"
...
function initBackgroundTools() {
  return Promise.all([
    runtime.runPromise(BashOutputTool.pipe(Effect.flatMap((info) => info.init()))),
    runtime.runPromise(BashKillTool.pipe(Effect.flatMap((info) => info.init()))),
    runtime.runPromise(BackgroundWaitTool.pipe(Effect.flatMap((info) => info.init()))),
  ])
}
...
describe("tool.bash", () => {
  ...
  each("runs commands in the background with output controls", async () => {
    await using tmp = await tmpdir()
    await Instance.provide({
      directory: tmp.path,
      fn: async () => {
        const bash = await initBash()
        const [outputTool, _killTool, waitTool] = await initBackgroundTools()
        const started = await Effect.runPromise(
          bash.execute(
            {
              command: "echo background-tool-ok",
              description: "Run background echo",
        ...
        const jobID = (started.metadata as Record<string, unknown>).job_id
        expect(typeof jobID).toBe("string")

        const waited = await Effect.runPromise(waitTool.execute({ job_id: jobID as string, timeout: 5_000 }, ctx))
        expect(waited.metadata.status).toBe("exited")

        const output = await Effect.runPromise(outputTool.execute({ job_id: jobID as string }, ctx))
        expect(output.metadata.status).toBe("exited")
        expect(output.output).toContain("background-tool-ok")
      },
    })
  })

  each("cancels background commands with bash_kill", async () => {
    await using tmp = await tmpdir()
    await Instance.provide({
      directory: tmp.path,
      fn: async () => {
        const bash = await initBash()
        const [_outputTool, killTool] = await initBackgroundTools()
        const command =
          process.platform === "win32"
            ? `${quote(process.execPath)} -e ${quote("setTimeout(() => {}, 10_000)")}`
            : `${quote(process.execPath)} -e ${squote("setTimeout(() => {}, 10_000)")}`
        const started = await Effect.runPromise(
          bash.execute(
            {
              command,
              description: "Run background sleep",
              run_in_background: true,
            },
            ctx,
          ),
        ...
        const killed = await Effect.runPromise(killTool.execute({ job_id: jobID as string }, ctx))
  ...
  if (process.platform === "win32") {
    ...
    for (const item of ps) {
      test(
        ...
        withShell(item, async () => {
          const key = "GAL_CODE_TEST_MISSING"
            ...
            fn: async () => {
              ...
              await expect(
                Effect.runPromise(
                  bash.execute(
                    {
                      command: "cat /tmp/gal-code-does-not-exist",
