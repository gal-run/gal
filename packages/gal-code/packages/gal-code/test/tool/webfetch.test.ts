...
function exec(args: { url: string; format: "text" | "markdown" | "html"; fullOutput?: boolean }) {
...
describe("tool.webfetch", () => {
  ...
  test("truncates large responses with metadata", async () => {
    ...
    await withFetch(
      ...
      async (url) => {
        await Instance.provide({
          directory: projectRoot,
          fn: async () => {
            const result = await exec({ url: new URL("/large.txt", url).toString(), format: "text" })
            expect(result.output).toContain("Response truncated")
            expect(result.output).toContain("6.00MB")
            expect(result.metadata.truncated).toBe(true)
            expect((result.metadata as { totalSize?: number }).totalSize).toBe(6 * 1024 * 1024)
          },
        })
      },
    )
  ...
  test("returns full output when fullOutput is true", async () => {
    const largeContent = "y".repeat(6 * 1024 * 1024) // 6MB
    await withFetch(
      () =>
        new Response(largeContent, {
          status: 200,
      ...
      async (url) => {
        await Instance.provide({
          directory: projectRoot,
          fn: async () => {
            const result = await exec({ url: new URL("/large.txt", url).toString(), format: "text", fullOutput: true })
