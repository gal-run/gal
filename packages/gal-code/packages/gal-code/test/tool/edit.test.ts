...
async function wait(promise: Promise<void>) {
  const ms = process.platform === "win32" ? 5_000 : 1_000
  await Promise.race([
    promise,
    new Promise((_, reject) => setTimeout(() => reject(new Error("timed out waiting for file event")), ms)),
  ])
}
    ...
    test("emits add event for new files", async () => {
      ...
      await Instance.provide({
        ...
        fn: async () => {
          ...
          const events: string[] = []
          const next = event()
          const unsubUpdated = await subscribeBus(FileWatcher.Event.Updated, () => {
            events.push("updated")
            next.done()
          })
