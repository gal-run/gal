...
import {
  ...
  type TuiSlotPlugin,
  type TuiTheme,
} from "@scheduler-systems/gal-code-plugin/tui"
...
function createThemeInstaller(
  ...
  return async (file) => {
    ...
    const local_dir =
      path.basename(source_dir) === ".gal/code"
        ? path.join(source_dir, "themes")
        : path.join(source_dir, ".gal/code", "themes")
...
function defaultPluginOrigin(state: RuntimeState, spec: string): Config.PluginOrigin {
  return {
    spec,
    scope: "local",
    source: state.api.state.path.config || path.join(state.directory, ".gal/code", "tui.json"),
  }
}
  ...
  async function load(api: Api) {
    ...
    await Instance.provide({
      ...
      fn: async () => {
        ...
        const records = Flag.GAL_CODE_PURE ? [] : (config.plugin_origins ?? [])
        if (Flag.GAL_CODE_PURE && config.plugin_origins?.length) {
