...
import galCodeTheme from "./theme/gal-code.json" with { type: "json" }
...
import type { TuiThemeCurrent } from "@scheduler-systems/gal-code-plugin/tui"
  ...
  init: (props: { mode: "dark" | "light" }) => {
    ...
    setStore(
      produce((draft) => {
        ...
        draft.lock = lock
        const active = config.theme ?? kv.get("theme", "gal-code")
        draft.active = typeof active === "string" ? active : "gal-code"
        draft.ready = false
      }),
    )
    ...
    const values = createMemo(() => {
      ...
      return resolveTheme(store.themes["gal-code"], store.mode)
    })
