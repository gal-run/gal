...
import type { GlobalEvent } from "@scheduler-systems/gal-code-sdk/v2"
...
declare global {
  const GAL_CODE_WORKER_PATH: string
}
...
async function target() {
  if (typeof GAL_CODE_WORKER_PATH !== "undefined") return GAL_CODE_WORKER_PATH
...
export const TuiThreadCommand = cmd({
  command: "$0 [project]",
  describe: "start gal-code tui",
  builder: (yargs) =>
    withNetworkOptions(yargs)
      .positional("project", {
        type: "string",
        describe: "path to start gal-code in",
      })
      .option("model", {
        type: "string",
        alias: ["m"],
