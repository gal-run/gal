...
export const PrCommand = cmd({
  command: "pr <number>",
  describe: "fetch and checkout a GitHub PR branch, then run gal-code",
  builder: (yargs) =>
      ...
      async fn() {
        ...
        if (prInfoResult.code === 0) {
          ...
          if (prInfoText.trim()) {
            ...
            // Check for gal-code session link in PR body
            if (prInfo && prInfo.body) {
              const sessionMatch = prInfo.body.match(/https:\/\/opncd\.ai\/s\/([a-zA-Z0-9_-]+)/)
              if (sessionMatch) {
                const sessionUrl = sessionMatch[0]
                UI.println(`Found gal-code session: ${sessionUrl}`)
                UI.println(`Importing session...`)

                const importResult = await Process.text(["gal-code", "import", sessionUrl], {
                  nothrow: true,
                })
        ...
        UI.println()
        UI.println("Starting gal-code...")
        UI.println()

        const galCodeArgs = sessionId ? ["-s", sessionId] : []
        const galCodeProcess = Process.spawn(["gal-code", ...galCodeArgs], {
          stdin: "inherit",
          stdout: "inherit",
          stderr: "inherit",
          cwd: process.cwd(),
        })
        const code = await galCodeProcess.exited
        if (code !== 0) throw new Error(`gal-code exited with code ${code}`)
      },
    })
  },
})
