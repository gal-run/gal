...
const AGENT_USERNAME = "gal-code-agent[bot]"
const AGENT_REACTION = "eyes"
const WORKFLOW_FILE = ".github/workflows/gal-code.yml"
      ...
      async fn() {
        {
          ...
          function printNextSteps() {
            ...
            prompts.outro(
              [
                ...
                step2,
                "",
                "    3. Go to a GitHub issue and comment `/gc summarize` to see the agent in action",
                "",
                "   Learn more about the GitHub agent - https://gal.run/docs/github/#usage-examples",
              ].join("\n"),
            )
          }
          ...
          async function installGitHubApp() {
            ...
            // Open browser
            const url = "https://github.com/apps/gal-code-agent"
            const command =
            ...
            async function getInstallation() {
              return await fetch(`https://api.gal.run/get_github_app_installation?owner=${app.owner}&repo=${app.repo}`)
                .then((res) => res.json())
...
jobs:
  "gal-code":
    if: |
      ...
      startsWith(github.event.comment.body, '/gc') ||
      contains(github.event.comment.body, ' /gal-code') ||
      startsWith(github.event.comment.body, '/gal-code')
    ...
    steps:
      ...
      - name: Run gal-code
        uses: gal-run/gal-code/github@latest${envStr}
        with:
...
export const GithubRunCommand = cmd({
  ...
  async handler(args) {
    await bootstrap(process.cwd(), async () => {
      ...
      const shareBaseUrl = isMock ? "https://dev.gal.run" : "https://gal.run"
      ...
      try {
        ...
        console.log("gal-code session", session.id)
      ...
      function normalizeOidcBaseUrl(): string {
        ...
        if (!value) return "https://api.gal.run"
      ...
      async function getUserPrompt() {
        ...
        const mentions = (process.env["MENTIONS"] || "/gal-code,/gc")
          .split(",")
      ...
      async function chat(message: string, files: PromptFiles = []) {
        console.log("Sending message to gal-code...")
      ...
      async function getOidcToken() {
        try {
          return await core.getIDToken("gal-code-github-action")
        } catch (error) {
      ...
      function generateBranchName(type: "issue" | "pr" | "schedule" | "dispatch") {
        ...
        if (type === "schedule" || type === "dispatch") {
          ...
          return `gal-code/${type}-${hex}-${timestamp}`
        }
        return `gal-code/${type}${issueId}-${timestamp}`
      }
      ...
      function footer(opts?: { image?: boolean }) {
        const image = (() => {
          ...
          return `<a href="${shareBaseUrl}/s/${shareId}"><img width="200" alt="${titleAlt}" src="https://social-cards.sst.dev/gal-code-share/${title64}.png?model=${providerID}/${modelID}&version=${session.version}&id=${shareId}" /></a>\n`
        })()
        const shareUrl = shareId ? `[gal-code session](${shareBaseUrl}/s/${shareId})&nbsp;&nbsp;|&nbsp;&nbsp;` : ""
    ...
    issue(number: $number) {
      ...
      function buildPromptDataForIssue(issue: GitHubIssue) {
        ...
        return [
          ...
          "- Git push and PR creation are handled AUTOMATICALLY by the gal-code infrastructure after your response",
