...
function headers(req: Request, extra?: HeadersInit) {
  ...
  out.delete("x-gal-code-directory")
  out.delete("x-gal-code-workspace")
  if (!extra) return out
...
const app = (upgrade: UpgradeWebSocket) =>
  new Hono().get(
    "/__workspace_ws",
    upgrade((c) => {
      const url = c.req.header("x-gal-code-proxy-url")
      const queue: Msg[] = []
...
export namespace ServerProxy {
  ...
  export function websocket(
    ...
    next.set("x-gal-code-proxy-url", socket(target.url))
