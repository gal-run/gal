...
export const DEPRECATED_PLUGIN_PACKAGES = ["gal-code-openai-codex-auth", "gal-code-copilot-auth"]
...
export async function checkPluginCompatibility(target: string, galCodeVersion: string, pkg?: PluginPackage) {
  if (!semver.valid(galCodeVersion) || semver.major(galCodeVersion) === 0) return
  const hit = pkg ?? (await readPluginPackage(target).catch(() => undefined))
  if (!hit) return
  const engines = hit.json.engines
  if (!isRecord(engines)) return
  const range = engines["gal-code"]
  if (typeof range !== "string") return
  if (!semver.satisfies(galCodeVersion, range)) {
    throw new Error(`Plugin requires gal-code ${range} but running ${galCodeVersion}`)
  }
}
