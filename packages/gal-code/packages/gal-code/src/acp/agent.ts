...
export namespace ACP {
  ...
  export async function init({ sdk: _sdk }: { sdk: GalCodeClient }) {
    return {
  ...
  export class Agent implements ACPAgent {
    ...
    async initialize(params: InitializeRequest): Promise<InitializeResponse> {
      ...
      const authMethod: AuthMethod = {
        description: "Run `gal auth login` in the terminal",
        name: "Login with GAL Code",
        id: "gal-code-login",
      }
    ...
    private async processMessage(message: SessionMessageResponse) {
      ...
      for (const part of message.parts) {
        if (part.type === "tool") {
          ...
          // GAL Code stores files internally as { type: "file", url, filename, mime }.
    ...
    private async loadSessionMode(params: LoadSessionRequest) {
      ...
      let currentVariant = this.sessionManager.getVariant(sessionId)
      if (currentVariant && !availableVariants.includes(currentVariant)) {
        this.sessionManager.setVariant(sessionId, undefined)
        currentVariant = undefined
      }
      if (!currentVariant) {
        const env = process.env.GAL_CODE_MODEL_VARIANT
        if (env && availableVariants.includes(env)) {
          this.sessionManager.setVariant(sessionId, env)
          currentVariant = env
        }
      }
  ...
  async function defaultModel(config: ACPConfig, cwd?: string): Promise<{ providerID: ProviderID; modelID: ModelID }> {
    ...
    if (listed?.selected) {
      return {
        providerID: ProviderID.make(listed.selected.providerID),
        modelID: ModelID.make(listed.selected.modelID),
      }
    }
    ...
    return { providerID: ProviderID["gal-code"], modelID: ModelID.make("zai-org/glm-4.7-maas") }
  }
