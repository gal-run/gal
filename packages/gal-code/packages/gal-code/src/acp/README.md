...
This directory contains a clean, protocol-compliant implementation of the [Agent Client Protocol](https://agentclientprotocol.com/) for GAL Code.
...
- **`session.ts`** - Session state management
  - Creates and tracks ACP sessions
   - Maps ACP sessions to internal GAL Code sessions
...
gal-code acp --cwd /path/to/project
```
...
```bash
GAL_CODE_ENABLE_QUESTION_TOOL=1 gal-code acp
```
...
- **Terminal Integration**: Full terminal support via GAL Code's bash tool
...
echo '{"jsonrpc":"2.0","id":1,"method":"initialize","params":{"protocolVersion":1}}' | gal-code acp
```
...
ACP sessions map cleanly to GAL Code's internal session model:
