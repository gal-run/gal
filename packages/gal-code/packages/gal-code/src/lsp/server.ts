...
import { Module } from "@scheduler-systems/gal-code-util/module"
...
export namespace LSPServer {
  ...
  export const Vue: Info = {
    ...
    async spawn(root) {
      ...
      const args: string[] = []
      if (!binary) {
        if (Flag.GAL_CODE_DISABLE_LSP_DOWNLOAD) return
  ...
  export const Ty: Info = {
    ...
    async spawn(root) {
      if (!Flag.GAL_CODE_EXPERIMENTAL_LSP_TY) {
        return undefined
      }
  ...
  export const JDTLS: Info = {
    ...
    async spawn(root) {
      ...
      const dataDir = await fs.mkdtemp(path.join(os.tmpdir(), "gal-code-jdtls-data"))
      return {
        process: spawn(
          java,
          [
