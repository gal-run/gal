...
import { Cause, Effect, Exit, Layer, Option, Context, Schedule, Stream } from "effect"
...
export namespace MCP {
  ...
  const own = (client: string, name: string) => {
    const server = sanitize(client)
    return sanitize(name).startsWith(server + "_")
  }

  const key = (client: string, name: string, seen: Set<string>) => {
    const server = sanitize(client)
    const tool = sanitize(name)
    const next = own(client, name) ? tool : `${server}_${tool}`
    if (!seen.has(next)) {
      seen.add(next)
      return next
    }
    const alt = `${server}__${tool}`
    if (!seen.has(alt)) {
      seen.add(alt)
      return alt
    }
  ...
  export interface Interface {
    ...
    readonly toolsForServer: (name: string) => Effect.Effect<Array<{ name: string; description?: string }>>
  ...
  export class Service extends Context.Service<Service, Interface>()("@gal-code/MCP") {}
    ...
    Effect.gen(function* () {
      ...
      const connectTransport = (transport: Transport, timeout: number) =>
        Effect.acquireUseRelease(
          ...
          (t) =>
            Effect.tryPromise({
              try: () => {
                const client = new Client({ name: "gal-code", version: Installation.VERSION })
              ...
              if (isAuthError) {
                ...
                if (lastError.message.includes("registration") || lastError.message.includes("client_id")) {
                  ...
                  return bus
                    .publish(TuiEvent.ToastShow, {
                      title: "MCP Authentication Required",
                      message: `Server "${key}" requires authentication. Run: gal-code mcp auth ${key}`,
                      variant: "warning",
                      duration: 8000,
                    })
                    .pipe(Effect.ignore, Effect.as(undefined))
      ...
      function watch(s: State, name: string, client: MCPClient, timeout?: number) {
        client.setNotificationHandler(ToolListChangedNotificationSchema, async () => {
          ...
          try {
            ...
            log.error("tool list change handler failed", { server: name, error: (err as Error)?.message })
          }
        })
      }

      const healthCheck = (s: State) =>
        Effect.gen(function* () {
          yield* Effect.repeat(
            Effect.gen(function* () {
              for (const [key, client] of Object.entries(s.clients)) {
                if (s.status[key]?.status !== "connected") continue
                const ok = yield* Effect.tryPromise({
                  try: () => client.listTools().then(() => true),
                  catch: (err) => {
                    log.warn("mcp health check failed", { key, error: (err as Error)?.message })
                    return false
                  },
                })
                if (!ok) {
                  s.status[key] = { status: "failed", error: "health check - server unreachable" }
                  delete s.clients[key]
                  delete s.defs[key]
                  log.error("mcp server marked failed by health check", { key })
                  yield* bus.publish(ToolsChanged, { server: key }).pipe(Effect.ignore)
                }
              }
            }),
      ...
      const state = yield* InstanceState.make<State>(
        Effect.fn("MCP.state")(function* () {
          ...
          yield* Effect.forEach(
            ...
            ([key, mcp]) =>
              Effect.gen(function* () {
                ...
                const result: CreateResult = yield* create(key, mcp).pipe(
                  Effect.catchCause((cause) => {
                    const msg = Cause.pretty(cause)
                    log.error("mcp init failed", { key, error: msg })
                    return Effect.succeed({ status: { status: "failed" as const, error: msg } } satisfies CreateResult)
                  }),
                )
                if (!result) return
          ...
          yield* healthCheck(s).pipe(Effect.forkScoped)
        ...
        yield* Effect.forEach(
          ...
          ([clientName, client]) =>
            Effect.gen(function* () {
              ...
              for (const mcpTool of [...listed].sort((a, b) => {
                const left = own(clientName, a.name)
                const right = own(clientName, b.name)
                if (left !== right) return left ? -1 : 1
                return sanitize(a.name).localeCompare(sanitize(b.name))
              })) {
                result[key(clientName, mcpTool.name, seen)] = convertMcpTool(mcpTool, client, timeout)
              }
            }),
          { concurrency: "unbounded" },
        )
      ...
      const toolsForServer = Effect.fn("MCP.toolsForServer")(function* (serverName: string) {
        const s = yield* InstanceState.get(state)
        const listed = s.defs[serverName]
        if (!listed) return []
  ...
  export const toolsForServer = async (name: string) => runPromise((svc) => svc.toolsForServer(name))
