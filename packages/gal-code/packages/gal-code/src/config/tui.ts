...
export namespace TuiConfig {
  ...
  function customPath() {
    return Flag.GAL_CODE_TUI_CONFIG
  }
  ...
  const state = Instance.state(async () => {
    let projectFiles = Flag.GAL_CODE_DISABLE_PROJECT_CONFIG
      ? []
    ...
    projectFiles = Flag.GAL_CODE_DISABLE_PROJECT_CONFIG
      ? []
      : await ConfigPaths.projectFiles("tui", Instance.directory, Instance.worktree)
    ...
    for (const dir of unique(directories)) {
      if (!ConfigPaths.local(dir)) continue
  ...
  async function load(text: string, configFilepath: string): Promise<Info> {
    ...
    // (mirroring the old gal-code.json shape) still get their settings applied.
