...
const TUI_SCHEMA_URL = "https://gal.run/tui.json"
...
/**
 * Migrates tui-specific keys (theme, keybinds, tui) from gal-code.json files
 * into dedicated tui.json files. Migration is performed per-directory and
 * skips only locations where a tui.json already exists.
 */
export async function migrateTuiConfig(input: MigrateInput) {
  const galCodeFilesResult = await galCodeFiles(input)
  for (const file of galCodeFilesResult) {
...
async function galCodeFiles(input: { directories: string[]; managed: string }) {
  const project = Flag.GAL_CODE_DISABLE_PROJECT_CONFIG
    ? []
    : await ConfigPaths.projectFiles("gal-code", Instance.directory, Instance.worktree)
  const files = [...project, ...ConfigPaths.fileInDirectory(Global.Path.config, "gal-code")]
  for (const dir of unique(input.directories)) {
    files.push(...ConfigPaths.fileInDirectory(dir, "gal-code"))
  }
  if (Flag.GAL_CODE_CONFIG) files.push(Flag.GAL_CODE_CONFIG)
  files.push(...ConfigPaths.fileInDirectory(input.managed, "gal-code"))
