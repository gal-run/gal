...
function streamFromGenerate(result: {
  ...
  return new ReadableStream({
    start(ctrl) {
      ...
      if (result.response.id || result.response.modelId || result.response.timestamp) {
        ctrl.enqueue({
...
export class OpenAICompatibleChatLanguageModel implements LanguageModelV3 {
  ...
  async doGenerate(options: LanguageModelV3CallOptions) {
    ...
    // Vultr streams `reasoning`, while Copilot uses `reasoning_text`.
        ...
        new TransformStream<ParseResult<z.infer<typeof this.chunkSchema>>, LanguageModelV3StreamPart>({
          ...
          transform(chunk, controller) {
            ...
            if (value.usage != null) {
              ...
              if (value.usage.reasoning_tokens != null) {
                usage.completionTokensDetails.reasoningTokens = value.usage.reasoning_tokens
              }
            ...
            // Vultr streams `reasoning`, while Copilot uses `reasoning_text`.
            const reasoning = delta.reasoning_text ?? delta.reasoning
            if (reasoning) {
