...
import type { SessionStatus } from "@scheduler-systems/gal-code-sdk/v2/client"
...
export function SessionRetry(props: { status: SessionStatus; show?: boolean }) {
  ...
  const statusPageUrl = createMemo(() => {
    const current = retry()
    return current?.statusPageUrl
  })
        ...
        <Card variant="error" class="error-card">
          <div class="flex items-start gap-2">
            ...
            <div class="min-w-0">
              ...
              <Show when={statusPageUrl()}>
                {(url) => (
                  <div data-slot="session-turn-retry-status" class="text-xs opacity-75 mt-1">
                    <a href={url()} target="_blank" rel="noopener noreferrer" class="underline hover:opacity-100">
