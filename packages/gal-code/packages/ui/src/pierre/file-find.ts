...
function clearHighlightFind() {
  ...
  if (!api) return
  api.delete("gal-code-find")
  api.delete("gal-code-find-current")
}
...
export function createFileFind(opts: CreateFileFindOptions) {
  ...
  const setHighlights = (ranges: Range[], currentIndex: number) => {
    ...
    if (active) api.set("gal-code-find-current", new Highlight(active))

    const rest = ranges.filter((_, i) => i !== currentIndex)
    if (rest.length > 0) api.set("gal-code-find", new Highlight(...rest))
    return true
  }
