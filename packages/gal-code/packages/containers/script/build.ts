...
const reg = process.env.REGISTRY ?? "ghcr.io/scheduler-systems"
...
const setup = async () => {
  ...
  if (list.includes("gal-code")) {
    await $`docker buildx use gal-code`
    return
  }
  await $`docker buildx create --name gal-code --use`
}
