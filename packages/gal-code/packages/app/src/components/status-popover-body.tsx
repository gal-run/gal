...
const useMcpAuthenticateMutation = () => {
  const sync = useSync()
  const sdk = useSDK()
  const language = useLanguage()
...
export function StatusPopoverBody(props: { shown: Accessor<boolean> }) {
  ...
  const authenticateMcp = useMcpAuthenticateMutation()
  const defaultServer = useDefaultServerKey(platform.getDefaultServer)
  const mcpNames = createMemo(() => Object.keys(sync.data.mcp ?? {}).sort((a, b) => a.localeCompare(b)))
  const mcpStatus = (name: string) => sync.data.mcp?.[name]?.status
  const mcpConnected = createMemo(() => mcpNames().filter((name) => mcpStatus(name) === "connected").length)
  const needsAuth = (name: string) => authRequiredStatuses.includes(mcpStatus(name) as (typeof authRequiredStatuses)[number])
  ...
  const pluginEmpty = createMemo(() => pluginEmptyMessage(language.t("dialog.plugins.empty"), "gal-code.json"))
              ...
              >
                <For each={mcpNames()}>
                  {(name) => {
                    ...
                    const requiresAuth = () => needsAuth(name)
                    return (
                      <button
                        type="button"
                        class="flex items-center gap-2 w-full h-8 pl-3 pr-2 py-1 rounded-md hover:bg-surface-raised-base-hover transition-colors text-left"
                        onClick={() => {
                          if (toggleMcp.isPending || requiresAuth()) return
                          toggleMcp.mutate(name)
                        }}
                        disabled={(toggleMcp.isPending && toggleMcp.variables === name) || requiresAuth()}
                      >
                        <div
                          classList={{
                        ...
                        <Show when={requiresAuth()}>
                          <Button
                            size="small"
                            disabled={authenticateMcp.isPending && authenticateMcp.variables === name}
                            onClick={(e: MouseEvent) => {
                              e.stopPropagation()
                              authenticateMcp.mutate(name)
                            }}
                          >
