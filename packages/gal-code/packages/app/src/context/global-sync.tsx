import type {
  ...
  Todo,
} from "@scheduler-systems/gal-code-sdk/v2/client"
...
function createGlobalSync() {
  ...
  const sdkCache = new Map<string, GalCodeClient>()
  ...
  const [globalStore, setGlobalStore] = createStore<GlobalStore>({
    ...
    project: projectCache.value.map(sanitizeProject),
    session_todo: {},
  ...
  const setProjects = (next: Project[] | ((draft: Project[]) => void)) => {
    ...
    setGlobalStore("project", next.map(sanitizeProject))
    cacheProjects()
  }
  ...
  if (projectInit instanceof Promise) {
    void projectInit.then(() => {
      ...
      const cached = projectCache.value.map(sanitizeProject)
