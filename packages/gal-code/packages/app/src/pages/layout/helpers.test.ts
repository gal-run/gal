...
import { type Session } from "@scheduler-systems/gal-code-sdk/v2/client"
import {
  childSessionOnPath,
  displayName,
...
describe("layout deep links", () => {
  test("parses open-project deep links", () => {
    expect(parseDeepLink("gal-code://open-project?directory=/tmp/demo")).toBe("/tmp/demo")
  })

  test("ignores non-project deep links", () => {
    expect(parseDeepLink("gal-code://other?directory=/tmp/demo")).toBeUndefined()
    expect(parseDeepLink("https://example.com")).toBeUndefined()
  })

  test("ignores malformed deep links safely", () => {
    expect(() => parseDeepLink("gal-code://open-project/%E0%A4%A%")).not.toThrow()
    expect(parseDeepLink("gal-code://open-project/%E0%A4%A%")).toBeUndefined()
  })
  ...
  test("ignores open-project deep links without directory", () => {
    expect(parseDeepLink("gal-code://open-project")).toBeUndefined()
    expect(parseDeepLink("gal-code://open-project?directory=")).toBeUndefined()
  })

  test("collects only valid open-project directories", () => {
    const result = collectOpenProjectDeepLinks([
      "gal-code://open-project?directory=/a",
      "gal-code://other?directory=/b",
      "gal-code://open-project?directory=/c",
    ])
    expect(result).toEqual(["/a", "/c"])
  })

  test("parses new-session deep links with optional prompt", () => {
    expect(parseNewSessionDeepLink("gal-code://new-session?directory=/tmp/demo")).toEqual({ directory: "/tmp/demo" })
    expect(parseNewSessionDeepLink("gal-code://new-session?directory=/tmp/demo&prompt=hello%20world")).toEqual({
      directory: "/tmp/demo",
      prompt: "hello world",
    })
  })
  ...
  test("ignores new-session deep links without directory", () => {
    expect(parseNewSessionDeepLink("gal-code://new-session")).toBeUndefined()
    expect(parseNewSessionDeepLink("gal-code://new-session?directory=")).toBeUndefined()
  })

  test("collects only valid new-session deep links", () => {
    const result = collectNewSessionDeepLinks([
      "gal-code://new-session?directory=/a",
      "gal-code://open-project?directory=/b",
      "gal-code://new-session?directory=/c&prompt=ship%20it",
    ])
  ...
  test("drains global deep links once", () => {
    const target = {
      __GAL_CODE__: {
        deepLinks: ["gal-code://open-project?directory=/a"],
      },
    } as unknown as Window & { __GAL_CODE__?: { deepLinks?: string[] } }

    expect(drainPendingDeepLinks(target)).toEqual(["gal-code://open-project?directory=/a"])
