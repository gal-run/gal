...
import { previewSelectedLines } from "@scheduler-systems/gal-code-ui/pierre/selection-bridge"
...
import { findLast } from "@scheduler-systems/gal-code-util/array"
import { createSessionTabs } from "@/pages/session/helpers"
import { extractPromptFromParts } from "@/utils/prompt"
import { UserMessage } from "@scheduler-systems/gal-code-sdk/v2"
...
export const useSessionCommands = (actions: SessionCommandContext) => {
  ...
  const detach = async () => {
    const sessionID = params.id
    if (!sessionID) return
    const result = await sdk.client.session.detach({ sessionID }).catch((err) => {
      showToast({
        title: "Move to background failed",
        description: err instanceof Error ? err.message : "No active foreground shell could be moved.",
      })
      return
    })
    ...
    showToast({
      ...
      description: `Job ${result.data.id} is still available for output, wait, or kill controls.`,
    })
  }
  ...
  const sessionCmds = () => [
    ...
    sessionCommand({
      ...
      description: "Detach the active shell and keep it running as a background job.",
      slash: "background",
      disabled: !params.id || status().type === "idle",
      onSelect: detach,
    }),
    sessionCommand({
      id: "session.fork",
