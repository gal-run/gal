...
const galCodeDir = path.join(repoDir, "packages", "gal-code")
...
const sandbox = await fs.mkdtemp(path.join(os.tmpdir(), "gal-code-e2e-"))
const keepSandbox = process.env.GAL_CODE_E2E_KEEP_SANDBOX === "1"

const serverEnv = {
  ...process.env,
  GAL_CODE_DISABLE_SHARE: process.env.GAL_CODE_DISABLE_SHARE ?? "true",
  GAL_CODE_DISABLE_LSP_DOWNLOAD: "true",
  GAL_CODE_DISABLE_DEFAULT_PLUGINS: "true",
  GAL_CODE_EXPERIMENTAL_DISABLE_FILEWATCHER: "true",
  GAL_CODE_TEST_HOME: path.join(sandbox, "home"),
  XDG_DATA_HOME: path.join(sandbox, "share"),
  XDG_CACHE_HOME: path.join(sandbox, "cache"),
  XDG_CONFIG_HOME: path.join(sandbox, "config"),
  XDG_STATE_HOME: path.join(sandbox, "state"),
  GAL_CODE_E2E_PROJECT_DIR: repoDir,
  GAL_CODE_E2E_SESSION_TITLE: "E2E Session",
  GAL_CODE_E2E_MESSAGE: "Seeded for UI e2e",
  GAL_CODE_E2E_MODEL: process.env.GAL_CODE_E2E_MODEL ?? "gal-code/gpt-5-nano",
  GAL_CODE_CLIENT: "app",
  GAL_CODE_STRICT_CONFIG_DEPS: "true",
...
const runnerEnv = {
  ...
  VITE_GAL_CODE_SERVER_HOST: "127.0.0.1",
  VITE_GAL_CODE_SERVER_PORT: String(serverPort),
...
try {
  ...
  if (seedExit !== 0) {
    ...
    process.env.AGENT = "1"
    process.env.GAL_CODE = "1"
    process.env.GAL_CODE_PID = String(process.pid)

    const log = await import("../../gal-code/src/util/log")
    const install = await import("../../gal-code/src/installation")
    await log.Log.init({
      print: true,
    ...
    const servermod = await import("../../gal-code/src/server/server")
    inst = await import("../../gal-code/src/project/instance")
    server = await servermod.Server.listen({ port: serverPort, hostname: "127.0.0.1" })
    console.log(`gal-code server listening on http://127.0.0.1:${serverPort}`)
