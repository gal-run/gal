...
test("collapsed sidebar popover stays open when archiving a session", async ({ page, project }) => {
  await project.open()
  const stamp = Date.now()

  const one = await project.sdk.session.create({ title: `e2e sidebar popover archive 1 ${stamp}` }).then((r) => r.data)
  const two = await project.sdk.session.create({ title: `e2e sidebar popover archive 2 ${stamp}` }).then((r) => r.data)
  ...
  try {
    ...
    const projectSwitch = page.locator(projectSwitchSelector(project.slug)).first()
    await expect(projectSwitch).toBeVisible()
    await projectSwitch.hover()
