...
import { base64Encode, checksum } from "@scheduler-systems/gal-code-util/encode"
...
export function createSdk(directory?: string, baseUrl = serverUrl) {
  return createGalCodeClient({ baseUrl, directory, throwOnError: true })
}
...
export function workspacePersistKey(directory: string, key: string) {
  ...
  return `gal-code.workspace.${head}.${sum}.dat:workspace:${key}`
}
