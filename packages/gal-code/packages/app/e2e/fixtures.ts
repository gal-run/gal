...
import type { Item, Reply, Usage } from "../../gal-code/test/lib/llm-server"
import { TestLLMServer } from "../../gal-code/test/lib/llm-server"
...
const seedModel = (() => {
  const [providerID = "gal-code", modelID = "glm-5"] = (
    process.env.GAL_CODE_E2E_MODEL ?? "gal-code/glm-5"
  ).split("/")
  return {
    providerID: providerID || "gal-code",
    modelID: modelID || "glm-5",
  }
})()
...
async function promptSend(page: Page) {
  return page
    .evaluate(() => {
      ...
      const sent = win["__gal-code_e2e"]?.prompt?.sent
      return {
  ...
  const send = async (text: string, input: { noReply: boolean; shell: boolean }) => {
    if (input.noReply) {
      ...
      const state = await page.evaluate(() => {
        const model = (window as E2EWindow)["__gal-code_e2e"]?.model?.current
...
async function seedStorage(
  ...
  await page.addInitScript(
    (args: {
      ...
      const key = "gal-code.global.dat:server"
      ...
      localStorage.setItem("gal-code.settings.dat:defaultServerUrl", args.serverUrl)
      ...
      localStorage.setItem("gal-code.global.dat:model", JSON.stringify({ recent: [args.model], user: [], variant: {} }))
    },
