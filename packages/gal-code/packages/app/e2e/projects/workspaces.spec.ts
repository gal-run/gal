...
import { base64Decode } from "@scheduler-systems/gal-code-util/encode"
...
test("non-git projects keep workspace mode disabled", async ({ page, project }) => {
  ...
  const nonGit = await fs.mkdtemp(path.join(os.tmpdir(), "gal-code-e2e-project-nongit-"))
  ...
  try {
    ...
    expect(path.basename(activeDir)).toContain("gal-code-e2e-project-nongit-")
