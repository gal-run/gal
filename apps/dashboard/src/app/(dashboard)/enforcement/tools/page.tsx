...
import { Shield, Wrench, Loader2, AlertCircle, Trash2, Clock, ToggleLeft, ToggleRight } from 'lucide-react'
import { useAuth } from '@/contexts/AuthContext'
import { useFeatureFlags } from '@/contexts/FeatureFlagsContext'
import { useSelectedWorkspace } from '@/hooks/useSelectedWorkspace'
import { useIsInternalWorkspace } from '@/hooks/useWorkspaceAudienceTier'
import { useToolPolicies } from '@/hooks/useEnforcement'
...
export default function ToolsPage() {
  ...
  const { items: policies, loading, error, refresh } = useToolPolicies(orgName)
  const [deleteConfirmId, setDeleteConfirmId] = useState<string | null>(null)
  const [deleting, setDeleting] = useState(false)

  const isInternalWorkspace = useIsInternalWorkspace()
  const isVisible = isInternalWorkspace && isPageVisibleForUser('enforcement-tools', userOrgs, selectedWorkspace)

  const handleDelete = useCallback(async (id: string) => {
    if (!orgName) return
    setDeleting(true)
    try {
      await api.deleteToolPolicy(orgName, id)
  ...
  if (!isVisible) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh] px-6">
        ...
        <p className="text-sm text-center max-w-md" style={{ color: 'var(--text-muted)' }}>Tool governance is only available to internal users.</p>
      </div>
    )
  }
  ...
  return (
    <div className="max-w-5xl mx-auto px-6 py-8">
      <div className="mb-8">
        <h1 className="text-2xl font-bold" style={{ color: 'var(--text-primary)' }}>Tool Governance</h1>
        <p className="text-sm mt-1" style={{ color: 'var(--text-muted)' }}>Manage tool restriction policies and audit logs</p>
      </div>
      ...
      {!loading && policies.length === 0 && (
        <div className="flex flex-col items-center justify-center py-16 rounded-xl" style={{ border: '1px dashed var(--border-primary)' }}>
          ...
          <p className="text-sm font-medium mb-1" style={{ color: 'var(--text-secondary)' }}>No tool policies configured</p>
          <p className="text-xs" style={{ color: 'var(--text-muted)' }}>All tool calls are allowed without restrictions.</p>
        </div>
      )}
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex flex-wrap gap-2 mt-2">
                    {policy.rules.map((rule, i) => (
                      <span key={i} className="text-xs px-1.5 py-0.5 rounded" style={{ background: 'var(--surface-raised)', color: ACTION_COLORS[rule.action] || 'var(--text-muted)' }}>
