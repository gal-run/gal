...
const viteContentConfigSource = readFileSync(
  join(__dirname, "../vite.content.config.ts"),
  "utf8",
);
...
describe("manifest/runtime regressions", () => {
  ...
  it("keeps command shortcuts declared for popup and workflow palette wiring", () => {
    ...
    expect(
      manifest.commands["open-command-palette"]?.suggested_key?.default,
    ).toBe("Ctrl+Shift+P");
    expect(
      manifest.commands["_execute_workflow_palette"]?.suggested_key?.default,
    ).toBe("Ctrl+Shift+G");
  });
