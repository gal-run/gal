/**
 * Regression tests for 'Save to GAL' optimistic UI with background
 * image capture.
 *
...
async function optimisticSave(
  img: { src: string; crossOrigin: boolean },
...
async function enhanceEntryInBackground(
  ...
  thumbnailFn: (dataUrl: string) => Promise<string | undefined>,
): Promise<void> {
  try {
...
/**
 * Full two-phase save-button handler ( pattern).
...
async function handleSaveButtonClick(
  ...
  fetchFn: (url: string) => Promise<string | undefined>,
  thumbnailFn: (dataUrl: string) => Promise<string | undefined>,
): Promise<{ entryId: string; phase2Promise: Promise<void> }> {
...
describe("Save to GAL — optimistic UI with background capture", () => {
  ...
  describe("Phase 1 — instant optimistic feedback", () => {
    ...
    it("shows 'saved' state immediately for same-origin images (no network needed)", async () => {
      ...
      const { entryId } = await handleSaveButtonClick(
        img,
        storage,
        (state) => feedbackCalls.push(state),
        fetchFnOk,
    ...
    it("shows 'saved' BEFORE Phase 2 background enhancement completes", async () => {
      ...
      const { phase2Promise } = await handleSaveButtonClick(
        img,
        storage,
        (state) => feedbackCalls.push(state),
        slowFetch,
  ...
  describe("Phase 2 — background enhancement (non-blocking)", () => {
    it("enhances the entry with a dataUrl when canvas capture failed", async () => {
      ...
      const { entryId, phase2Promise } = await handleSaveButtonClick(
        img,
        storage,
        (state) => feedbackCalls.push(state),
        fetchFnOk,
      ...
      // Before phase 2
      const before = (storage.get("entries") ?? []).find(
        (e) => e.id === entryId,
      );
      ...
      const after = (storage.get("entries") ?? []).find(
        (e) => e.id === entryId,
      );
    ...
    it("returns undefined from background fetch when fetch returns undefined — entry retains imageUrl fallback", async () => {
      ...
      const entry = (storage.get("entries") ?? []).find(
        (e) => e.id === entryId,
      );
  ...
  describe("storage consistency", () => {
    it("each save produces a unique entry id — multiple saves do not clobber each other", async () => {
      ...
      const { entryId: id1 } = await handleSaveButtonClick(
        img1,
        storage,
        () => {},
        fetchFnOk,
      ...
      const { entryId: id2 } = await handleSaveButtonClick(
        img2,
        storage,
        () => {},
        fetchFnOk,
