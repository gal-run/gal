/**
 * Regression tests for single-writer cache to eliminate palette flicker.
 *
        ...
        get: vi.fn(async (key: string | string[] | null) => {
          ...
          if (Array.isArray(key)) {
            return Object.fromEntries(
              key.map((k) => [k, localStore[k]]).filter(([, v]) => v !== undefined),
            );
          }
...
function makeConfigResponse(
  commandNames: string[],
): { approved: boolean; commands: Array<{ name: string; content: string }> } {
  return {
    approved: true,
...
describe("regression — single-writer cache / palette flicker prevention", () => {
    ...
    it("two concurrent REFRESH_COMMANDS for the same org result in exactly one network call", async () => {
      ...
      const results = await Promise.all([
        new Promise<unknown>((resolve) => {
          messageHandler(
            { type: "REFRESH_COMMANDS", orgName: "acme" },
            sender,
            resolve,
          );
        }),
    ...
    it("two concurrent REFRESH_COMMANDS for *different* orgs each make their own network call", async () => {
      ...
      await Promise.all([
        new Promise<void>((resolve) => {
          messageHandler(
            { type: "REFRESH_COMMANDS", orgName: "org-alpha" },
            sender,
            () => resolve(),
          );
        }),
        new Promise<void>((resolve) => {
          messageHandler(
            { type: "REFRESH_COMMANDS", orgName: "org-beta" },
            sender,
            () => resolve(),
          );
        }),
  ...
  describe("no transient empty during cache refresh (atomic update)", () => {
          ...
          if ("cachedCommands" in items) {
            try {
              commandSnapshots.push(
                JSON.parse(items.cachedCommands as string) as Record<string, unknown[]>,
              );
            } catch {
    ...
    it("other orgs' cache entries survive while one org is being refreshed", async () => {
      ...
      await new Promise<void>((resolve) => {
        messageHandler(
          { type: "REFRESH_COMMANDS", orgName: "org-a" },
          sender,
          () => resolve(),
        );
      });
      ...
      expect((finalCache!["org-b"] as Array<{ name: string }>)[0].name).toBe(
        "cmd-b",
      );
    });
  });
  ...
  describe("cache-on-error protection (single-writer contract)", () => {
    it("an API 500 error does not wipe existing cached commands for the org", async () => {
      ...
      expect(
        (finalCache!.acme as Array<{ name: string }>)[0].name,
      ).toBe("saved-cmd");
    });
  ...
  describe("concurrent writes to distinct orgs — no interleaving or corruption", () => {
    it("three simultaneous refreshes for three orgs all land correct data without corruption", async () => {
      ...
      await Promise.all(
        ["org-x", "org-y", "org-z"].map(
          (orgName) =>
            new Promise<void>((resolve) => {
              messageHandler(
                { type: "REFRESH_COMMANDS", orgName },
                sender,
                () => resolve(),
              );
            }),
      ...
      for (const [org, expectedNames] of Object.entries(orgData)) {
        ...
        const actual = (
          finalCache![org] as Array<{ name: string }>
        ).map((c) => c.name);
    ...
    it("no write produces an interleaved or partially-merged command list", async () => {
      ...
      await Promise.all([
        ...
        new Promise<void>((resolve) => {
          messageHandler(
            { type: "REFRESH_COMMANDS", orgName: "org-b" },
            sender,
            () => resolve(),
          );
        }),
      ...
      const orgANames = (
        finalCache!["org-a"] as Array<{ name: string }>
      ).map((c) => c.name);
      ...
      const orgBNames = (
        finalCache!["org-b"] as Array<{ name: string }>
      ).map((c) => c.name);
