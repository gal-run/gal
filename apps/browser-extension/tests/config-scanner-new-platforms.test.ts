/**
 * Regression tests for Chrome Extension config scanning for Kling,
 ...
 *     name, return shape, and uses the documented selectors from issue .
 *
...
const higgsfieldSource = readFileSync(
  join(SCANNERS_DIR, "higgsfield-scanner.ts"),
  "utf8",
);
...
describe("URL routing — hasScannerForUrl correctly routes new platforms", () => {
  ...
  it("recognises app.klingai.com subdomain as a supported platform", () => {
    expect(hasScannerForUrl("https://app.klingai.com/global/video/new")).toBe(
      true,
    );
  });
  ...
  it("recognises higgsfield.ai as a supported platform", () => {
    expect(
      hasScannerForUrl("https://higgsfield.ai/cinema-studio"),
    ).toBe(true);
  });

  it("recognises app.higgsfield.ai subdomain as a supported platform", () => {
    expect(
      hasScannerForUrl("https://app.higgsfield.ai/cinema-studio"),
    ).toBe(true);
  });
  ...
  it("recognises aistudio.google.com as a supported platform (AI Studio)", () => {
    expect(hasScannerForUrl("https://aistudio.google.com/prompts/new_chat")).toBe(
      true,
    );
  });
...
describe("Kling scanner source contracts", () => {
  ...
  it("returns a 'Not on an image or video generation page' summary when mode is null", () => {
    expect(klingSource).toContain(
      "Not on an image or video generation page.",
    );
...
describe("Higgsfield scanner source contracts", () => {
  ...
  it("returns a graceful fallback summary when no Cinema Studio config is detected", () => {
    expect(higgsfieldSource).toContain(
      "No Cinema Studio config detected",
    );
  });
