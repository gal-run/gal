...
function StatusDetails({
  ...
  return (
    <div className="mt-1 p-2.5 rounded-lg bg-gray-800/50 border border-gray-700 text-xs space-y-2">
      {/* Sync details */}
      {syncStatus && (
        <div className="space-y-1">
          ...
          {syncStatus.lastSyncAt && (
            <p className="text-gray-500 pl-[18px]">
              Last sync: {formatRelativeDate(syncStatus.lastSyncAt)}
            </p>
          )}
          {syncStatus.configVersion && (
            <p className="text-gray-500 pl-[18px]">
              Version: v{syncStatus.configVersion}
            </p>
          )}
          {syncStatus.driftDetected && syncStatus.driftFiles.length > 0 && (
            <p className="text-gray-400 pl-[18px]">
          ...
          {!syncStatus.synced && !syncStatus.driftDetected && (
            <p className="text-gray-500 pl-[18px]">
      ...
      {/* Sync Copilot hint */}
      {syncHint && (
        <div className="p-1.5 rounded bg-gal-accent/10 border border-gal-accent/20">
      ...
      <div className="border-t border-gray-700/50 pt-2 space-y-1.5">
        ...
        {isScanning && scanProgress && (
          <div className="p-1.5 rounded bg-gal-accent/5 border border-gal-accent/20">
            ...
            {scanProgress.currentRepo && (
              <p className="text-[#737373] mt-0.5 truncate">
        ...
        {/* Stale Scan Alert */}
        {scanFreshnessLoaded && isOlderThan7Days(lastScanAt) && !isScanning && (
          <div className="p-1.5 rounded bg-amber-500/10 border border-amber-500/20 flex items-center gap-1.5">
            <AlertTriangle className="w-3 h-3 text-amber-400 flex-shrink-0" />
  ...
  return (
    <div className="relative">
      ...
      >
        ...
        {expanded ? (
          <ChevronUp className="w-3 h-3 ml-auto flex-shrink-0 text-gray-600" />
        ) : (
          <ChevronDown className="w-3 h-3 ml-auto flex-shrink-0 text-gray-600" />
        )}
      </button>
