...
export async function acceptTerms(termsVersion = "1.0"): Promise<void> {
  try {
    await apiRequest<{ termsAcceptedAt: string; termsVersion: string }>(
      "/user/accept-terms",
...
async function apiRequest<T>(
  ...
  for (let attempt = 0; attempt < MAX_RETRIES; attempt++) {
    ...
    if (response.status === 429 && attempt < MAX_RETRIES - 1) {
      const degradation = classifyHttpStatus(response.status, response.headers);
      if (degradation) {
        storeDegradation(degradation).catch(() => {});
      }
 ...
 */
export async function getApprovedConfig(
  orgName: string,
  platform: string = "claude",
): Promise<ApprovedConfig | null> {
  try {
    const query = platform ? `?platform=${platform}` : "";
    const response = await apiRequest<ApprovedConfigApiResponse>(
      `/organizations/${orgName}/approved-config${query}`,
    );
 ...
 */
export async function getApprovedConfigResult(
  orgName: string,
  platform?: string,
): Promise<FetchResult<Command[]>> {
  const query = platform ? `?platform=${platform}` : "";
  const result = await apiRequestResult<ApprovedConfigApiResponse>(
    `/organizations/${orgName}/approved-config${query}`,
  );
 ...
 */
export async function getSyncPreflightHint(
  orgName: string,
  platform: string = "claude",
): Promise<SyncCopilotHintResponse | null> {
  try {
    return await apiRequest<SyncCopilotHintResponse>(
 ...
 */
export async function generateApprovedConfigProposal(
  orgName: string,
  request: {
    platform: string;
    rationale: string;
  ...
  try {
    return await apiRequest<GenerateApprovedConfigProposalResult>(
      `/organizations/${orgName}/approved-config/proposals/generate`,
      {
        method: "POST",
        body: JSON.stringify({
          ...request,
 ...
 */
export async function getDiscoveryInsight(
  orgName: string,
  request: {
    repo: string;
    filePath: string;
  ...
  try {
    return await apiRequest<DiscoveryInsightResponse>(
      `/organizations/${orgName}/discovery-intelligence/insights`,
      {
        method: "POST",
        body: JSON.stringify({
          ...request,
...
export async function recordGovernanceOverride(params: {
  ...
  try {
    return await apiRequest<{ id: string; timestamp: string }>(
...
export async function getProposals(
  ...
  try {
    ...
    const response = await apiRequest<{ proposals: ConfigProposal[] }>(
      `/api/orgs/${encodeURIComponent(orgName)}/proposals?${params.toString()}`,
    );
 ...
 */
export async function getDiscoveredConfigs(
  orgName: string,
  options?: { type?: string },
  ...
  try {
    ...
    return await apiRequest<DiscoveredConfigsResponse>(
...
export async function triggerScan(orgName: string): Promise<ScanResult> {
  try {
    return await apiRequest<ScanResult>(
      `/scan/${encodeURIComponent(orgName)}`,
      { method: "POST" },
    );
  } catch (error) {
...
export async function getScanProgress(orgName: string): Promise<ScanProgress> {
  try {
    return await apiRequest<ScanProgress>(
      `/scan/${encodeURIComponent(orgName)}/progress`,
    );
  } catch {
    return {
      status: "idle",
 ...
 */
export function isPageVisibleForOrg(
  flags: FeatureFlagResponse | null,
  pageId: string,
 ...
 */
export async function getEnforcementPolicies(
  orgName: string,
