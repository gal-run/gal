 ...
 */
async function fetchCommandsFromAPI(
  token: string,
  orgName: string,
): Promise<FetchResult<Command[]>> {
  try {
    const response = await fetch(
      `${API_BASE_URL}/organizations/${encodeURIComponent(orgName)}/approved-config?platform=claude`,
      {
        headers: {
          Authorization: `Bearer ${token}`,
          "Content-Type": "application/json",
...
async function fetchCommandsDeduped(
  token: string,
  orgName: string,
): Promise<FetchResult<Command[]>> {
 ...
 */
async function applyCacheResult(
  orgName: string,
  result: FetchResult<Command[]>,
): Promise<void> {
  const now = Date.now();
...
export async function handleGitHubAuth(): Promise<void> {
  ...
  try {
    ...
    if (!response.ok) {
      const errMsg = reportAuthError(
        new Error(data?.message || data?.error || "Failed to start auth"),
        requestId,
      );
    ...
    if (!data?.authUrl) {
      const errMsg = reportAuthError(
        new Error("No auth URL provided"),
        requestId,
      );
      await chrome.storage.local.set({ galAuthError: errMsg });
      return;
    ...
    if (!responseUrl) {
      const errMsg = reportAuthError(
        new Error("Auth cancelled or no response received"),
        requestId,
      );
      await chrome.storage.local.set({ galAuthError: errMsg });
      return;
    ...
    if (!token || !userId || !userLogin) {
      const errMsg = reportAuthError(
        new Error("Invalid auth response"),
        requestId,
      );
      await chrome.storage.local.set({ galAuthError: errMsg });
      return;
