...
function captureImageViaCanvas(
...
function findVisibleTargetInRoot(
  root: ParentNode,
...
function findClipboardPasteTarget(): HTMLElement | null {
  ...
  if (active instanceof HTMLElement && active.shadowRoot) {
    ...
    const targetInActiveShadow = findVisibleTargetInRoot(
      active.shadowRoot,
  ...
  for (const shadowRoot of findOpenShadowRoots()) {
    const shadowTarget = findVisibleTargetInRoot(
      shadowRoot,
      SHADOW_PASTE_TARGET_SELECTORS,
    );
 ...
 */
export async function transferToCurrentPlatform(
...
function injectSaveButton(img: HTMLImageElement, platform: string): void {
  ...
  if (platform === "chatgpt") {
    ...
    const overlayLeft = container?.querySelector<HTMLElement>(
