...
const jsonLd = {
  ...
  name: 'GAL - Gemini Code Assist Integration',
      ...
      <div className="max-w-[1376px] mx-auto">
        ...
        <div className="grid md:grid-cols-3 gap-8">
          <div className="bg-white p-6 rounded-lg border border-gray-100">
            ...
            <h3 className="text-lg font-medium text-black mb-2">Inconsistent .gemini configs</h3>
  ...
  const features = [
    {
      icon: RefreshCw,
      title: 'Config Sync',
      description: 'Pull approved .gemini/settings.json, .gemini/config.json, and custom instructions with a single command. Keep every developer on the same configuration.',
    },
    {
      icon: Shield,
    ...
    },
    {
      ...
      description: 'GAL wraps your Gemini workflow. One command to sync, one command to verify compliance, zero friction for developers using Google Cloud tools.',
    },
    {
      icon: Users,
      title: 'Team Management',
      description: 'Invite team members, manage access levels, and ensure everyone runs the same baseline Gemini configuration across all projects.',
    },
  ]
  ...
  const steps = [
    {
      ...
      description: 'Install the GAL GitHub App to connect your repositories. GAL auto-discovers all Gemini configurations across your codebase.',
    },
    {
      step: '2',
      title: 'Define Approved Config',
      description: "Create your organization's approved .gemini/settings.json, .gemini/config.json, and custom instructions in the GAL dashboard.",
    },
    {
      step: '3',
      title: 'Developers Sync',
      description: 'Team members run `gal sync --agent gemini` to get the approved configuration. Updates are distributed instantly when policies change.',
    },
  ]
...
# Pull the organization's approved Gemini configuration
gal sync --agent gemini

# Output:
# ✓ .gemini/settings.json updated (v2 → v3)
# ✓ .gemini/config.json synced (4 new custom rules)
# ✓ Gemini instructions aligned with org policy
# 
# Sync complete. Gemini Code Assist is now on approved baseline.`}
          </pre>
        </div>
      </div>
    </section>
  ...
  const useCases = [
    {
      ...
      description: 'Define rules for what context Gemini Code Assist can access. Surface sensitive-data exposure risks (active prevention coming in v1.0). Aim for every Gemini session follows security policy.',
    },
    {
      title: 'Engineering Leads',
      description: 'Distribute coding standards via .gemini/config.json. Share custom instructions for common Google Cloud workflows. Onboard new developers faster with pre-configured settings.',
    },
    {
...
function FAQSection() {
  const faqs = [
    {
      question: 'Does GAL replace Gemini Code Assist?',
      answer: 'No. GAL is a governance layer that works WITH Gemini Code Assist. Your developers still use Gemini normally. GAL ensures they all use the same approved configuration and policies.',
    },
    {
      question: 'What files does GAL sync for Gemini?',
      answer: 'GAL syncs .gemini/settings.json (editor settings), .gemini/config.json (project configuration), and custom instructions and rules defined for your Gemini Code Assist environment.',
    },
    {
      question: 'How do developers update their config?',
      answer: 'Developers run `gal sync --agent gemini` to get the latest approved configuration. You can also set up automated sync on a schedule or via CI/CD.',
    },
    {
