...
describe("content script — generation and clipboard helpers are scoped to supported hosts", () => {
 ...
 expect(contentScriptSource).toContain('if (hostname.includes("gemini.google.com"))',);
 expect(contentScriptSource).toContain('initAssetClipboard("gemini").catch(() => {});',);
 expect(contentScriptSource).toContain('else if (hostname.includes("aistudio.google.com"))',);
 expect(contentScriptSource).toContain('initAssetClipboard("ai-studio").catch(() => {});',);
 });
 ...
 expect(contentScriptSource).toContain('if (location.hostname.includes("gemini.google.com")) {',);
...
describe("privileged clipboard/browser APIs remain isolated behind the service worker", () => {
 ...
 expect(serviceWorkerSource).toContain('message.type === "GAL_FETCH_IMAGE_MAIN_WORLD"',);
 expect(serviceWorkerSource).toContain('message.type === "GAL_DOWNLOAD_IMAGE"',);
 });
