...
.mockReturnValue("https://ext.chromiumapp.org/github-callback"),
 ...
 expect((error as Error).message).toBe("The requested redirect URI is not allowed.",);
 expect(tags).toEqual({
 ...
 vi.mocked(chrome.identity.launchWebAuthFlow).mockImplementation((_options, callback) => {
 setTimeout(() => {
 ...
 vi.mocked(chrome.identity.launchWebAuthFlow).mockImplementation((_options, callback) => {
 setTimeout(() => {
 callback("https://ext.chromiumapp.org/github-callback?token=tok-123&userId=user-123&login=octocat",);
 }, 0);
 },);
