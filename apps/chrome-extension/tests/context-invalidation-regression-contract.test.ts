...
const guardianSource = readFileSync(join(__dirname, "../src/content/generation-guardian.ts"),
 "utf8",);
...
describe("context invalidation regression contracts", () => {
 ...
 expect(contentSource).toContain('chrome.storage.session.set({ activeGpt: JSON.stringify(gptInfo) }).catch(() => {});',);
 expect(contentSource).toContain('chrome.storage.session.set({ activeGpt: JSON.stringify(null) }).catch(() => {});',);
 expect(contentSource).toContain('chrome.storage.session.set({ activeGem: JSON.stringify(gemInfo) }).catch(() => {});',);
 expect(contentSource).toContain('chrome.storage.session.set({ activeGem: JSON.stringify(null) }).catch(() => {});',);
 });
