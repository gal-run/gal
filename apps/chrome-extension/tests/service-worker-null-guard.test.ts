/**
 * Regression tests — null guard on fetchCommandsFromAPI in service worker.
 *
 * Verifies:
...
describe("regression — null guard on fetchCommandsFromAPI", () => {
 ...
 await expect(Promise.resolve(alarmListener!({ name: "refresh-commands" })),).resolves.not.toThrow();
 });
 ...
 // (this is the regression scenario from)
 vi.mocked(fetch)
...
.mockResolvedValueOnce({
 ok: true,
 // JSON body is null — null guard introduced must handle this
 json: async () => null,
 } as Response);
...
.mockResolvedValueOnce({
 ...
 await expect(Promise.resolve(alarmListener!({ name: "keepalive" })),).resolves.not.toThrow();
