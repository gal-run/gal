...
describe("content script regressions", () => {
 ...
 expect(contentScriptSource).toContain("Cleanup legacy floating button injected by old extension versions",);
 });

 it("refreshes cached commands whenever the workflow palette opens to avoid stale entries", () => {
 expect(contentScriptSource).toContain("Always refresh commands on open (fix for stale workflow palette)",);
