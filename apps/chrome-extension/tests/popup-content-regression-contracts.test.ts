...
const commandCardSource = readFileSync(join(__dirname, "../src/components/CommandCard.tsx"),
 "utf8",);

const syncStatusSource = readFileSync(join(__dirname, "../src/components/SyncStatusCard.tsx"),
 "utf8",);

const apiSource = readFileSync(join(__dirname, "../src/lib/api.ts"),
 "utf8",);
...
describe("popup/content regression contracts", () => {
 ...
 expect(workflowPaletteSource).toContain("const minRight = Math.max(EDGE_MARGIN, vw - PALETTE_WIDTH - EDGE_MARGIN);",);
 expect(workflowPaletteSource).toContain("paletteStyle.right = `${Math.min(rawRight, minRight)}px`;",);
 });
 ...
 expect(syncStatusSource).toContain("Scan data is over 7 days old. Click &quot;Scan Now&quot; to refresh.",);
 ...
 expect(apiSource).toContain("Get discovered configs from Firestore cache (via API).",);
 expect(apiSource).toContain("`/organizations/${encodeURIComponent(orgName)}/discovered-configs?${params}`",);
