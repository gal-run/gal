/**
 * Regression tests — Chrome Extension detailed usage telemetry.
 *
 * Verified behaviours:
...
const telemetrySource = readFileSync(join(__dirname, "../src/lib/telemetry.ts"),
 "utf8",);

const serviceWorkerSource = readFileSync(join(__dirname, "../src/background/service-worker.ts"),
 "utf8",);
...
describe("regression — telemetry source contracts (static)", () => {
 ...
 expect(workflowPaletteSource).toContain('"extension.workflow_selected"',);
 });

 it("extension.workflow_injected event is tracked in WorkflowPalette", () => {
 expect(workflowPaletteSource).toContain('"extension.workflow_injected"',);
 });

 it("extension.workflow_dismissed event is tracked in WorkflowPalette", () => {
 expect(workflowPaletteSource).toContain('"extension.workflow_dismissed"',);
 });
 ...
 expect(telemetrySource).toContain("eventQueue.length >= MAX_BATCH_SIZE",);
 });
});
...
describe("regression — initTelemetry runtime", () => {
 ...
 const { initTelemetry, trackEvent, flushEvents } = await import("../src/lib/telemetry");
 await initTelemetry();
 ...
 const { initTelemetry } = await import("../src/lib/telemetry");
 await initTelemetry();
 expect(chromeMock.alarms.create).toHaveBeenCalledWith("gal-telemetry-flush",
...
describe("regression — handleFlushAlarm", () => {
 ...
 const { initTelemetry, trackEvent, handleFlushAlarm } = await import("../src/lib/telemetry");
 await initTelemetry();
 trackEvent("extension.session_start");
...
describe("regression — trackEvent never throws", () => {
 ...
 expect(() =>
 trackEvent("extension.session_start", { nonExistentAttr: undefined }),).not.toThrow();
 });
