/**
 * Regression tests — clipboard fetch via chrome.scripting.executeScript
 * bypasses Gemini CSP.
 *
 ...
 */
async function fetchViaExecuteScriptMainWorld(tabId: number | undefined,
 imageUrl: string,
...
describe("clipboard fetch via executeScript bypasses Gemini CSP", () => {
 ...
 await fetchViaExecuteScriptMainWorld(42,
 ...
 await fetchViaExecuteScriptMainWorld(99,
 ...
 await fetchViaExecuteScriptMainWorld(10,
 IMAGE_URL,
 { executeScript: executeScriptMock },);
 ...
 await fetchViaExecuteScriptMainWorld(5,
 ...
 await fetchViaExecuteScriptMainWorld(3,
 ...
 const result = await fetchViaExecuteScriptMainWorld(42,
 "https://lh3.googleusercontent.com/img.png",
 scripting,);
 ...
 const result = await fetchViaExecuteScriptMainWorld(7,
 ...
 const { dataUrl } = await fetchViaExecuteScriptMainWorld(1,
 ...
 const result = await fetchViaExecuteScriptMainWorld(20,
 "https://lh3.googleusercontent.com/img.png",
 scripting,);
 ...
 const result = await fetchViaExecuteScriptMainWorld(30,
 "https://lh3.googleusercontent.com/img.png",
 scripting,);
 ...
 const result = await fetchViaExecuteScriptMainWorld(40,
 "https://lh3.googleusercontent.com/img.png",
 scripting,);
 ...
 const scripting: ChromeScripting = {
 executeScript: vi.fn().mockRejectedValue(new Error("Cannot access contents of url"),),
 };
 ...
 const scripting: ChromeScripting = {
 executeScript: vi.fn().mockRejectedValue(new DOMException("Cannot access a chrome:// URL",
 "SecurityError",),),
 };

 await expect(fetchViaExecuteScriptMainWorld(8,
 "https://lh3.googleusercontent.com/img.png",
 scripting,),).resolves.toEqual({ dataUrl: null });
 });

 it("tab-navigated-away error from executeScript is caught — returns { dataUrl: null }", async () => {
 const scripting: ChromeScripting = {
 executeScript: vi.fn().mockRejectedValue(new Error("The tab was closed"),),
 };

 const result = await fetchViaExecuteScriptMainWorld(9,
 "https://lh3.googleusercontent.com/img.png",
 scripting,);
 ...
 const result = await fetchViaExecuteScriptMainWorld(undefined,
 "https://lh3.googleusercontent.com/img.png",
 { executeScript: executeScriptMock },);
 ...
 fetchViaExecuteScriptMainWorld(i + 1,
 "https://lh3.googleusercontent.com/img.png",
 scripting,),),);
 ...
 const result = await fetchViaExecuteScriptMainWorld(undefined, // popup sender: no tab
 "https://lh3.googleusercontent.com/img.png",
 { executeScript: executeScriptMock },);
