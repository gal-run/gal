/**
 * Regression tests — clean up trigger text and restore focus on palette dismiss.
 *
 * Verifies:
...
describe("stripTriggerText — textarea value cleanup", () => {
 ...
 expect(workflowPaletteSource).toContain('new Event("input", { bubbles: true })',);
 });
...
describe("handleClose trigger text cleanup contract", () => {
 ...
 const handleCloseBlock = contentSource.slice(handleCloseStart,
...
describe("Escape key dismiss path", () => {
 ...
 expect(workflowPaletteSource).toContain('document.removeEventListener("keydown", handleEscape, { capture: true })',);
 });
