...
export function CommandList({ commands, onCopy }: CommandListProps) {
 ...
 {filteredCommands.length === 0 ? (<div className="text-center py-8 text-gray-500">
 {searchQuery
 ? "No workflows match your search"
 : "No workflows available"}
 </div>) : (filteredCommands.map((command) => (<CommandCard key={command.id} command={command} onCopy={onCopy} />)))}
 </div>
 </div>);
}
