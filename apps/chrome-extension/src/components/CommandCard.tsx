...
export function CommandCard({ command, onCopy }: CommandCardProps) {
 ...
 return (<div className="p-3 bg-gray-800/50 rounded-lg border border-gray-700 hover:border-gal-accent/30 transition-colors group">
 <div className="flex items-start justify-between gap-3">
 <div className="flex-1 min-w-0">
 <h3 className="text-sm font-semibold text-white truncate">
 {command.name}
 </h3>
 {command.description && (<p className="text-xs text-gray-400 mt-1 line-clamp-2">
 {command.description}
 </p>)}
 {command.tags && command.tags.length > 0 && (<div className="flex gap-1 mt-2 flex-wrap">
 {command.tags.map((tag) => (<span
 key={tag}
 ...
 >
 {copied ? (<Check className="w-4 h-4 text-gal-accent" />) : (<Copy className="w-4 h-4" />)}
 </button>
 </div>
 </div>);
}
