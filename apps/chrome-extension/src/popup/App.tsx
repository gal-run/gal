...
export function App() {
 ...
 const [syncHint, setSyncHint] = useState<SyncCopilotHintResponse | null>(null,);
 ...
 const [activeTab, setActiveTab] = useState<"workflows" | "clipboard">("workflows",);
 ...
 const handleSyncChange = (changes: Record<string, chrome.storage.StorageChange>,) => {
 if (Object.prototype.hasOwnProperty.call(changes, "inFieldButtonDisabled")) {
 setInFieldButtonDisabled(changes.inFieldButtonDisabled.newValue === true,);
 }
 };
 ...
 useEffect(() => {
 const handleCmdCacheChange = (changes: Record<string, chrome.storage.StorageChange>,) => {
 ...
 const timestamps = [chatgpt?.scannedAt, gemini?.scannedAt].filter((t): t is number => typeof t === "number",);
 ...
 setPlatformScanChatgpt(reparse(changes.scan_chatgpt?.newValue as string | undefined),);
 }
 if (Object.prototype.hasOwnProperty.call(changes, "scan_gemini")) {
 setPlatformScanGemini(reparse(changes.scan_gemini?.newValue as string | undefined),);
 }
 };
 ...
 if (raw) {
 try {
 setActiveDesignProject(JSON.parse(raw) as ActiveDesignProjectSummary | null,);
 } catch {
 ...
 const handleStorageChange = (changes: Record<string, chrome.storage.StorageChange>,) => {
 if (Object.prototype.hasOwnProperty.call(changes, "activeDesignProject")) {
...
.catch(() => {});
 ...
 const handleWarningChange = (changes: Record<string, chrome.storage.StorageChange>,) => {
 ...
 */
 ...
 const cachedAuth = await getCacheEntry<AuthStatus>("cachedAuthStatus",
 ...
 const cachedOrgs = await getCacheEntry<Organization[]>("cachedOrganizations",
 ...
 const cachedCmds = await getCacheEntry<Record<string, Command[]>>("cachedCommands",
 ...
 */
 const revalidateInBackground = useCallback(async (cachedAuth: { data: AuthStatus; timestamp: number } | null,
 ...
 await setCacheEntry<AuthStatus>("cachedAuthStatus",
 "cachedAuthStatusTimestamp",
 freshAuth,);
 ...
 await setCacheEntry<Organization[]>("cachedOrganizations",
 "cachedOrganizationsTimestamp",
 freshOrgs,);
 ...
 const cachedCmds = await getCacheEntry<Record<string, Command[]>>("cachedCommands",
 "cachedCommandsTimestamp",);
 const cachedSync = await getCacheEntry<Record<string, SyncStatus>>("cachedSyncStatus",
 ...
 */
 ...
 try {
 const allCached = JSON.parse(cached.cachedDiscoveredConfigs as string,) as Record<string, { cachedAt?: string }>;
 ...
 existingCache = JSON.parse(cached.cachedDiscoveredConfigs as string,) as Record<string, unknown>;
 }
 } catch {
 ...
 */
 const loadOrgDataFromNetwork = async (orgName: string,
...
.catch(() => {});
 ...
 await setCacheEntry<Record<string, SyncCopilotHintResponse>>("cachedSyncHint",
 ...
 await setCacheEntry<Record<string, SyncStatus>>("cachedSyncStatus",
 "cachedSyncStatusTimestamp",
 syncStatusCache,);
...
.catch(() => {
 ...
 setOptionalHostPermissionError("Could not request site access. Try again.",);
 }
 };
...
.catch(() => {});
 ...
 if (!tab?.id) {
 setTransferError("No active tab found. Please focus a browser tab and try again.",);
 return;
 }
...
.catch(() => null); // content script not loaded
 ...
 setTransferError('GAL needs site access before it can run on this page. Click "Enable on this site" and retry.',);
 } else {
 setTransferError("Content script not loaded on this page. Reload the page and try again.",);
 }
 } else if (!response.ok && response.error) {
 setTransferError(response.error);
 }
 } catch (err) {
 setTransferError(err instanceof Error ? err.message : "Transfer failed — unknown error.",);
 } finally {
 ...
 */
 ...
 setTransferError("Clipboard write permission denied. Please allow clipboard access and try again.",);
 } else {
...
.replace(/\/\*$/, "")
 ...
 if (isLoading) {
 return (<div className="w-[400px] h-[600px] bg-[#0a0a0a] flex items-center justify-center">
 <div className="animate-spin rounded-full h-8 w-8 border-b-2 border-gal-accent" />
 </div>);
 }

 if (!isAuthenticated) {
 return (<div className="w-[400px] h-[600px] bg-[#0a0a0a]">
 <LoginView onLoginSuccess={handleLoginSuccess} />
 </div>);
 }

 return (<div className="w-[400px] h-[600px] bg-[#0a0a0a] flex flex-col">
 {/* Header */}
 ...
 {activeGpt && activePlatform === "chatgpt" ? (<span
 ...
 </span>) : activeGem && activePlatform === "gemini" ? (<span
 ...
 </span>) : platformBadgeLabel ? (<span className="inline-flex items-center px-2 py-0.5 rounded-full bg-[rgba(255,255,255,0.08)] border border-[rgba(255,255,255,0.12)] text-[10px] text-[#a1a1a1] font-medium">
 {platformBadgeLabel}
 </span>) : null}
 </div>
 ...
 {/* Workspace Switcher */}
 {organizations.length > 0 && (<WorkspaceSwitcher
 ...
 {/* User info */}
 {user && (<div className="mt-2 flex items-center justify-between">
 <div className="flex items-center gap-2">
 {user.avatarUrl && (<img
 src={user.avatarUrl}
 alt={user.login}
 ...
 {activeDesignProject && (<div className="mt-2 p-2.5 rounded-lg bg-[rgba(255,255,255,0.04)] border border-[rgba(255,255,255,0.08)]">
 ...
 ? `${Math.round((activeDesignProject.completedScenes /
 ...
 Clipboard
 {clipboardEntries.length > 0 && (<span className="px-1 py-0.5 rounded-full bg-gal-accent/20 text-gal-accent text-[9px] font-bold leading-none">
 ...
 {/* Storage quota warning */}
 {storageWarning && (<div className="mb-2 px-2.5 py-1.5 rounded-lg bg-amber-500/10 border border-amber-500/30">
 <p className="text-[11px] text-amber-400">{storageWarning}</p>
 </div>)}
 {optionalHostPermissionNeeded && optionalHostLabel && (<div className="mb-2 px-2.5 py-2 rounded-lg bg-blue-500/10 border border-blue-500/30">
 ...
 {optionalHostPermissionError && (<span className="text-[10px] text-red-400">
 {optionalHostPermissionError}
 </span>)}
 </div>
 </div>)}
 {activeTab === "workflows" ? (error ? (<div className="overflow-y-auto h-full p-3 bg-red-500/10 border border-red-500/30 rounded-lg">
 <p className="text-sm text-red-400">{error}</p>
 </div>) : (<CommandList commands={commands} onCopy={handleCommandCopy} />)) : (/* Clipboard tab */
 <div className="overflow-y-auto h-full">
 {/* Transfer error toast */}
 {transferError && (<div className="flex items-start gap-2 mx-1 mb-2 p-2 rounded-lg bg-red-500/10 border border-red-500/30">
 ...
 {!clipboardLoaded ? (<div className="flex items-center justify-center h-full">
 <Loader2 className="w-5 h-5 animate-spin text-[#737373]" />
 </div>) : clipboardEntries.length === 0 ? (<div className="flex flex-col items-center justify-center h-full gap-3 text-center px-4">
 ...
 {clipboardEntries.map((entry) => (<div
 key={entry.id}
 ...
 {entry.prompt && (<p className="text-[10px] text-[#a1a1a1] truncate leading-tight">
 {entry.prompt}
 </p>)}
 </div>
 ...
 >
 {transferringId === entry.id ? (<Loader2 className="w-2.5 h-2.5 animate-spin" />) : (<Upload className="w-2.5 h-2.5" />)}
 Use here
 </button>
 <button
 ...
 >
 {copiedImageId === entry.id ? (<Check className="w-2.5 h-2.5" />) : (<Copy className="w-2.5 h-2.5" />)}
 ...
 <button
 onClick={async () => {
 const updated = clipboardEntries.filter((e) => e.id !== entry.id,);
 await chrome.storage.local.set({
