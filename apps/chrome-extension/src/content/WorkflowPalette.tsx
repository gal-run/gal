 ...
 */
function findFirstPlaceholder(text: string,): { start: number; end: number } | null {
 ...
 */
function injectText(element: HTMLElement,
 text: string,
 triggerText?: string,): void {
 ...
 const ta = (element.tagName === "TEXTAREA"
 ? element
 : document.querySelector('textarea[aria-label="Enter a prompt"]')) as HTMLTextAreaElement | null;
 if (ta) {
 const nativeSetter = Object.getOwnPropertyDescriptor(HTMLTextAreaElement.prototype,
 "value",)!.set!;
 ...
 const endOffset = Math.min(startOffset + (placeholder.end - placeholder.start),
 node.length,);
 ...
 */
export function stripTriggerText(element: HTMLElement,
 triggerText: string,): void {
 if (element.contentEditable === "true") {
...
function HighlightedText({ segments }: { segments: HighlightSegment[] }) {
 return (<>
 {segments.map((seg, i) =>
 seg.highlighted ? (<strong
 key={i}
 style={{ color: "var(--gal-accent)", fontWeight: 700 }}
 >
 {seg.text}
 </strong>) : (<span key={i}>{seg.text}</span>),)}
 </>);
}
...
function SyncStatusIndicator({
 ...
 switch (syncState) {
 case "fresh":
 return (<span style={{...indicatorStyle, color: "var(--gal-text-muted)" }}>
 Updated{" "}
 {lastSuccessAt ? formatRelativeTime(lastSuccessAt) : "just now"}
 </span>);
 case "stale":
 return (<span style={{...indicatorStyle, color: "var(--gal-text-secondary)" }}>
 ...
 case "error":
 return (<span style={{...indicatorStyle, color: "#f59e42" }}>
 <AlertTriangle style={{ width: 11, height: 11 }} />
 {lastError === "auth_expired" ? "Re-login required" : "Sync failed"}
 {onRetry && lastError !== "auth_expired" && (<button onClick={onRetry} style={retryBtnStyle} title="Retry sync">
...
export function WorkflowPalette({
 ...
 // Handle injection
 const handleInject = useCallback(async (command: Command) => {
 ...
 const getMatchesForItem = useCallback((itemId: string,
 field: string,): readonly [number, number][] | undefined => {
 if (!displayItems.isSearching) return undefined;
 const result = displayItems.searchResults.find((r) => r.item.id === itemId,);
 ...
 >
 {displayItems.items.length === 0 ? (<div
 style={{
 textAlign: "center",
 padding: "32px 16px",
 ...
 >
 {isFetching && !query ? (<p style={{ margin: "0 0 12px", fontSize: "14px" }}>
 Loading workflows&hellip;
 </p>) : syncMetadata?.syncState === "error" &&
 ...
 <p style={{ margin: "0 0 12px", fontSize: "14px" }}>
 {query ? (<>No workflows match &ldquo;{query}&rdquo;</>) : ("No workflows found")}
 </p>
 <button
 style={{
 display: "inline-flex",
 ...
 {cmd.tags && cmd.tags.length > 0 && (<div
 style={{
 display: "flex",
 gap: "4px",
 marginTop: "6px",
