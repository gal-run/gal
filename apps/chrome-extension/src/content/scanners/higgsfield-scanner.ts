...
function readActiveCamera(): HiggsFieldCamera | null {
 ...
 const cameraSelector = document.querySelector('[data-testid="camera-selector"],.camera-selector,.camera-setup-btn',);
 if (cameraSelector) {
...
function readAspectRatio(): string | null {
 ...
 const activeRatio = document.querySelector('.aspect-ratio-selector [aria-pressed="true"],.aspect-ratio-group.active, [data-testid="aspect-ratio"].selected',);
 if (activeRatio) {
...
function readResolution(): string | null {
 ...
 const resEl = document.querySelector('.resolution-selector [aria-pressed="true"],.resolution-group.active, [data-testid="resolution"].selected',);
 if (resEl) {
...
function readGridLayout(): string | null {
 ...
 const gridEl = document.querySelector('.grid-layout-selector [aria-pressed="true"],.grid-selector.active, [data-testid="grid-layout"]',);
 if (gridEl) {
...
function readSavedCameras(): HiggsFieldCamera[] {
 ...
 const savedItems = document.querySelectorAll('.saved-cameras-list [role="option"],.camera-presets-saved.preset-item,.saved-tab.camera-item',);
 ...
 if (cameras.length === 0) {
 const presetButtons = document.querySelectorAll(".camera-preset-btn,.saved-camera-btn",);
