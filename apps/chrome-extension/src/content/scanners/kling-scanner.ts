...
function readModel(): string {
 ...
 const modelEl = document.querySelector(".el-select.ai-web-select-model-version.name-new",);
...
function readPrompt(): string {
 ...
 const textarea = document.querySelector<HTMLTextAreaElement>('textarea[placeholder*="prompt" i], textarea[placeholder*="describe" i]',);
...
function readGenerationMode(): string {
 ...
 const activeMode = document.querySelector('.mode-select [aria-pressed="true"],.mode-selector.active,.mode-tab.active, [class*="mode-select"].is-active',);
 if (activeMode?.textContent?.trim()) {
 return activeMode.textContent.trim();
 }

 // Check for an el-radio-group with selected item
 const radioSelected = document.querySelector('.el-radio-group.el-radio-button.is-active.el-radio-button__inner,.mode-group.selected',);
...
function readAspectRatio(): string {
 const activeRatio = document.querySelector('.ratio-select [aria-pressed="true"],.ratio-selector.active,.aspect-ratio-item.active, [class*="ratio"].is-active, [class*="aspect"].selected',);
 if (activeRatio?.textContent?.trim()) {
 return activeRatio.textContent.trim();
 }

 // Look for aspect ratio in any button group
 const buttons = document.querySelectorAll('[class*="ratio"] button, [class*="aspect"] button',);
...
function readOutputCount(): number | null {
 const countEl = document.querySelector('.output-count.active,.image-count.is-active, [class*="num-select"].active',);
 ...
 const numInput = document.querySelector<HTMLInputElement>('input[type="number"][class*="count"],.el-input-number input',);
 if (numInput) {
...
function readDuration(): string {
 const activeDuration = document.querySelector('.duration-select.active,.duration-selector.is-active, [class*="duration"].selected',);
