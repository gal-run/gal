...
function readModel(): string {
 ...
 const modelEl = document.querySelector('ms-model-selector [data-test-id="model-name"]',);
 ...
 const runSettingsModel = document.querySelector('.run-settings-section.model-name,.run-settings.model-selector-text',);
...
function readTemperature(): number | null {
 ...
 const sliders = document.querySelectorAll<HTMLElement>('.run-settings [role="slider"][aria-valuenow],.settings-section [role="slider"][aria-valuenow]',);
 ...
 const tempInput = document.querySelector<HTMLInputElement>('input[aria-label*="temperature" i], input[data-test-id*="temperature" i]',);
 if (tempInput) {
...
function readSystemInstructions(): string {
 const textarea = document.querySelector<HTMLTextAreaElement>("textarea.in-run-settings",);
 if (textarea) return textarea.value.trim();

 // Fallback: look for system instructions in other common locations
 const fallback = document.querySelector<HTMLTextAreaElement>('textarea[aria-label*="system" i], textarea[placeholder*="system" i]',);
...
function readMatSelectByLabel(label: string): string | null {
 const select = document.querySelector<HTMLElement>(`mat-select[aria-label="${label}"]`,);
...
function readTools(): AIStudioToolState[] {
 ...
 const toolItems = document.querySelectorAll(".settings-item.settings-tool,.tool-toggle-item",);

 toolItems.forEach((item) => {
 const nameEl = item.querySelector(".setting-label,.tool-name, label,.settings-item-label",);
 const name = nameEl?.textContent?.trim() ?? "unknown";

 // Check for Angular Material toggle or standard checkbox
 const toggle = item.querySelector<HTMLElement>('mat-slide-toggle, [role="switch"], input[type="checkbox"]',);
