 ...
 */
async function waitForStorage(key: string,
 timeoutMs: number,): Promise<string | null> {
 const deadline = Date.now() + timeoutMs;
 while (Date.now() < deadline) {
 const value = await getStorageData(key as Parameters<typeof getStorageData>[0],);
 ...
 */
let openWorkflowPalette: (anchor: HTMLElement | null,
 ...
 */
async function tryOpenWorkflowPalette(anchor: HTMLElement | null,
 triggerText?: string,): Promise<void> {
 ...
 */
 ...
 const cachedCmds = await getCacheEntry<Record<string, CommandType[]>>("cachedCommands",
 ...
 (async () => {
 const cached = await getCacheEntry<Record<string, CommandType[]>>("cachedCommands",
 "cachedCommandsTimestamp",);
 ...
 useEffect(() => {
 openWorkflowPalette = (newAnchor: HTMLElement | null,
...
function injectInFieldIcon(input: HTMLElement): void {
 ...
 // anchor (e.g. the download button on Google AI Studio).
 ...
 const nativeButtons = anchor.querySelectorAll<HTMLElement>("button:not(.gal-infield-icon), [role='button']:not(.gal-infield-icon)",);
...
chrome.runtime.onMessage.addListener((message) => {
 if (message.type === "INSERT_WORKFLOW_TEXT" &&
 ...
 if (!target) {
 console.warn("[GAL] INSERT_WORKFLOW_TEXT: no chat input found on this page",);
 return;
 }
...
async function getExistingScan(platform: "chatgpt" | "gemini",): Promise<ScanItem[]> {
 try {
