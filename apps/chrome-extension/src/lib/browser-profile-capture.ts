...
async function containsPermission(permissions: chrome.permissions.Permissions,
 permissionsApi: PermissionsLike,): Promise<boolean> {
 try {
...
function unsupportedTabSummary(tab: chrome.tabs.Tab,
 reason: string,): BrowserProfileTabSummary | null {
...
export async function diagnoseMissingBrowserAuth(tabUrl: string,
 deps?: {
 permissionsApi?: PermissionsLike;
 },): Promise<string> {
 try {
 ...
 const cookiesPermissionGranted = await containsPermission({ permissions: ["cookies"] },
 permissionsApi,);
...
export async function describeBrowserProfileTab(tab: chrome.tabs.Tab,
 deps?: {
 permissionsApi?: PermissionsLike;
 runtime?: RuntimeLike;
 ...
 if (!parsedUrl) {
 return unsupportedTabSummary(tab,
 ...
 const existingPermission = await containsPermission({ origins: [`${parsedUrl.protocol}//${parsedUrl.hostname}/*`] },
 permissionsApi,);
...
export async function listCapturableBrowserProfileTabs(deps?: {
 ...
 const summaries = (await Promise.all(tabs.map((tab) =>
...
export async function extractBrowserProfileFromTab(tab: chrome.tabs.Tab,
 deps?: {
 permissionsApi?: PermissionsLike;
 ...
 throw new Error(await diagnoseMissingBrowserAuth(tab.url, {
 permissionsApi,
 }),);
 }
 ...
 storageState: JSON.stringify(buildPlaywrightStorageState(chromeCookies, localStorageOrigin),),
 cookieCount: chromeCookies.length,
 localStorageEntryCount,
 };
}

export async function saveBrowserProfileFromTab(tabId: number,
 profileName?: string,
 deps?: {
 permissionsApi?: PermissionsLike;
 runtime?: RuntimeLike;
 ...
 throw new Error(permResult.error || `Permission denied for ${summary.hostname}. Cannot capture browser auth.`);
 }
