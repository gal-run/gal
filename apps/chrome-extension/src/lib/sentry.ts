...
export function initSentry(): void {
 ...
 if (!dsn) {
 console.warn("[GAL] Sentry is disabled because VITE_SENTRY_DSN is not configured.",);
 return;
 }
...
export function captureExceptionWithTags(error: unknown,
 tags: SentryTags,): void {
 if (!initialized) return;
