 ...
 */
export async function fetchCookiesForTab(tabUrl: string,
 cookiesApi: ChromeCookiesApi = chrome.cookies,): Promise<chrome.cookies.Cookie[]> {
 ...
 */
export async function extractLocalStorageForTab(tabId: number | undefined,
 scriptingApi: ChromeScriptingApi = chrome.scripting,): Promise<ExportedLocalStorageOrigin | null> {
...
.filter((entry): entry is ExportedLocalStorageEntry =>
 !!entry &&
...
export function buildPlaywrightStorageState(chromeCookies: chrome.cookies.Cookie[],
 localStorageOrigin: ExportedLocalStorageOrigin | null,): { cookies: Array<Record<string, unknown>>; origins: ExportedLocalStorageOrigin[] } {
 return {
