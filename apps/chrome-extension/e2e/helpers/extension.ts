...
export async function getExtensionId(context: BrowserContext): Promise<string> {
 ...
 if (!id) {
 throw new Error("Could not determine extension ID. Make sure the extension is built: pnpm --filter @gal/chrome-extension build",);
 }
 return id;
}
 ...
 */
export async function openExtensionPopup(context: BrowserContext,
 extensionId: string,): Promise<Page> {
 ...
 */
export async function waitForContentScriptReady(page: Page,
 timeout = 10_000,): Promise<void> {
 await page.waitForFunction(() => {
 // The content script creates a shadow host; check for any gal element
 return (document.querySelector('[id^="gal-"]') !== null ||
 ...
 */
export async function isContentScriptInjected(page: Page,
 timeout = 5_000,): Promise<boolean> {
 try {
 ...
 */
export async function waitForSaveToGalButton(page: Page,
